...
let build_symbol_hint_index (ws:t) : (string, string list) Hashtbl.t * (string, string list) Hashtbl.t =
  ...
  let add_doc_hints (doc:Document.t) : unit =
    ...
    | None -> ()
    | Some compool ->
        let ckey = normalize_name compool in
        if ckey <> "" then (
          let exp = sem_exports_of_doc doc in
          Hashtbl.iter (fun sym _ -> add_compool_hint values ~symbol_key:sym ~compool_key:ckey) exp.values;
          Hashtbl.iter (fun sym _ -> add_compool_hint types ~symbol_key:sym ~compool_key:ckey) exp.types
        )
  in
...
let validate_semantics (ws:t) (doc:Document.t) : T.Diagnostic.t list =
  ...
  | Some prog ->
      ...
      let hint_values, hint_types = symbol_hint_index ws in
