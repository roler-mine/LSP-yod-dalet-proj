...
let take_ident_from (s:string) (i:int) : string option =
  ...
  if !j >= n then None
  else if not (is_ident_char s.[!j]) then None
  else (
    let k = ref !j in
...
let scan_compool_def ~(text:string) : string option =
  (* crude but practical: search for START COMPOOL <ident> on any line *)
  let lines = String.split_on_char '\n' text in
  let rec go = function
    | [] -> None
    | line :: rest ->
        let u = uppercase (normalize_spaces line) in
        if contains_substring ~haystack:u ~needle:"START"
           && contains_substring ~haystack:u ~needle:"COMPOOL"
        then (
          (* token-ish scan: find "START", then "COMPOOL", then next identifier-like token *)
          let parts =
            u
          ...
          let rec find = function
            | "START" :: "COMPOOL" :: name :: _ ->
                let name = String.trim name in
                if name = "" then go rest else Some name
            | _ :: tl -> find tl
            | [] -> go rest
          in
          find parts
