...
%{
  ...
  (* --- basic error tracking for multi-error recovery --- *)
  let diags : (Ast.Loc.t * string) list ref = ref []

  let add_diag sp ep msg =
    diags := (loc sp ep, msg) :: !diags

  let take_diags () =
    let xs = List.rev !diags in
    diags := [];
    xs

  let bad_decl sp ep msg =
    add_diag sp ep msg;
    n sp ep (Ast.DDirective { name = nid sp ep "__ERROR__"; args = [] })

  let bad_stmt sp ep msg =
    add_diag sp ep msg;
    mk_block sp ep []
%}
...
/* Error recovery at top level: skip garbage up to a terminator. */
module_item:
  | ds=decl_item     { List.map (fun d -> Ast.TopDecl d) ds }
  | s=statement      { [Ast.TopStmt s] }
  | error _t=terminator
      { [Ast.TopStmt (bad_stmt $startpos $endpos "syntax error (top-level)")] }
  ;
...
decl_item:
  ...
  | ds=proc_decl     { ds }
  | error _t=terminator
      { [bad_decl $startpos $endpos "syntax error (declaration)"] }
  ;
...
/* ---------- directives ----------
   Supports:
     ICOMPOOL ('A','B');
     !COMPOOL 'DATABASE' GETVALUE,ERROR;
   We parse "!" as part of the directive name for indexing.
*/
directive_decl:
  ...
  | BANG name=ident args=directive_args_opt _t=terminator
      {
        let nm = "!" ^ name.v in
        let d = Ast.DDirective { name = nid $startpos $endpos nm; args } in
        n $startpos $endpos d
      }
  ;

/* Allow optional parentheses around arg lists, e.g. ('A','B') */
directive_args_opt:
  | /* empty */ { [] }
...
/* For directives we accept comma-separated OR whitespace-separated args,
   because common forms are:  'DB' GETVALUE,ERROR
*/
directive_args:
...
directive_arg:
  ...
  | LPAREN xs=directive_args_opt RPAREN
      {
        (* flatten nested parens *)
        let _ = xs in
        nid $startpos $endpos "__group__"
      }
  ;
...
/* Make init explicit to avoid ambiguity with type sizing:
   ITEM X B(10) = 0;
*/
init_opt:
  | /* empty */ { None }
...
/* type_spec now supports both:
     B 10
     C(80)
     I(1:10)
*/
type_spec:
  | base=ident suff=type_suffix
      {
        match suff with
...
type_suffix:
  | /* empty */ { [] }
  | ds=type_sizes_seq { ds }
  | LPAREN ds=dim_list_opt RPAREN { ds }
  ;

type_sizes_seq:
  | e=expr { [e] }
  | e=expr rest=type_sizes_seq { e :: rest }
  ;
...
proc_decl:
  ...
  | error _t=terminator
      { [bad_decl $startpos $endpos "syntax error (procedure)"] }
  ;
...
proc_body_opt:
  ...
  | BEGIN error END
      {
        let _ = bad_stmt $startpos $endpos "syntax error (procedure body)" in
        Some ([], mk_block $startpos $endpos [])
      }
  ;
...
rev_decl_section:
  ...
  | xs=rev_decl_section error _t=terminator
      { [bad_decl $startpos $endpos "syntax error (local declaration)"] :: xs }
  ;
...
statement:
  ...
  | error _t=terminator { bad_stmt $startpos $endpos "syntax error (statement)" }
  ;
...
compound_stmt:
  ...
  | BEGIN error END
      { bad_stmt $startpos $endpos "syntax error (block)" }
  ;
...
/* allow resync for statement lists on terminators */
simple_or_control_stmt:
  ...
  | error _t=terminator { bad_stmt $startpos $endpos "syntax error (simple/control)" }
  ;
...
expr:
  ...
  | lhs=expr POW rhs=expr
      {
        (* avoid needing Ast.BPow in your AST: represent as call *)
        let callee = nid $startpos $endpos "__pow__" in
        n $startpos $endpos (Ast.ECall { callee; args = [lhs; rhs] })
      }
