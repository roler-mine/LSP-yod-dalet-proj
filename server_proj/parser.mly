/* jovial_parser.mly — Menhir parser for JOVIAL (J73) subset + LSP spans.

   This grammar is designed for:
   - Robust parsing for editor/LSP use (error productions, ERROR tokens).
   - “Good enough” completeness for modules/compools, directives, DEFINE,
     declarations, statements, and expressions.

   It intentionally accepts some constructs more permissively (e.g. treating
   a standalone variable reference statement as SCallRef) so the parser keeps
   going; semantic passes can tighten and produce diagnostics.
*/

%{
  open Ast

  let loc (sp:Lexing.position) (ep:Lexing.position) (node:'a) : 'a located =
    { node; span = (sp, ep) }

  let loc0 node = loc $startpos $endpos node
%}
...
interlude_item:
  ...
  | ERROR(msg) { `Dir (loc0 { kind=D_ILIST; args=[ArgString ("LEXERR:"^msg)] }) }
...
define_stmt:
  | DEFINE name=ident v=DEFINE_STRING SEMI
      { loc0 { name; value=v } }
  | DEFINE name=ident ERROR(msg) SEMI
