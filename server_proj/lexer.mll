{
  ...
  let () =
    ...
    (* Define capability (Chapter 18) *)
    add "DEFINE" DEFINE;

    (* Declarations / modules / externals *)
    add "DEF" DEF;
    add "REF" REF;
    add "RENT" RENT;
    add "REC" REC;
    ...
    (* Type-ish and built-ins frequently used in declarations *)
    add "BIT" BIT;
    add "BITSIZE" BITSIZE;
    ...
    (* Boolean/pointer literals *)
    add "TRUE" TRUE;
    add "FALSE" FALSE;
    add "NULL" NULL;

    (* Built-in functions that are reserved words in J73 *)
    add "LOC" LOC;
    add "NEXT" NEXT;
    add "FIRST" FIRST;
    add "LAST" LAST;
  ...
  (* Classify identifiers:
     ...
     - Otherwise, single-letter becomes LETTER (index-letter), longer becomes NAME.
  *)
  let classify_ident (s:string) : token =
    let u = uppercase s in
    match Hashtbl.find_opt kw u with
    | Some tok ->
        (* Special: DEFINE puts lexer in AfterDefineDQ mode until a DEFINE_STRING
           is produced or the statement ends. *)
        (match tok with
  ...
  (* Update span from the current lexeme and return token. *)
  let emit (lexbuf:Lexing.lexbuf) (t:token) : token =
    set_span (Lexing.lexeme_start_p lexbuf) (Lexing.lexeme_end_p lexbuf);
    t

  (* If we are in AfterDefineDQ mode but hit ';' before seeing a DEFINE_STRING,
     reset state to avoid misclassifying later comments as define strings. *)
  let on_stmt_terminator () =
    dq_state := NormalDQ

  (* Error token helper: sets span and returns ERROR(msg). *)
...
(* bit literal: <size>B'<digits 0..3>'  size is 1..5 *)
let bit_size = ['1'-'5']
let bit_digits = ['0'-'3']+

rule raw_token (lexbuf : Lexing.lexbuf) : parser.token =
  parse
  (* whitespace *)
  | wsp+         { raw_token lexbuf }
  | nl           { Lexing.new_line lexbuf; raw_token lexbuf }

  (* percent-comment: % ... %  (can include newlines; cannot include %) *)
  | '%'          {
...
and percent_comment (startp:Lexing.position) buf =
  parse
  | '%' {
      let endp = Lexing.lexeme_end_p lexbuf in
      set_span startp endp;
...
and dq_comment (startp:Lexing.position) buf =
  parse
  | '"' {
...
*)
and define_string (startp:Lexing.position) buf =
  parse
  | "\"\"" {
      Buffer.add_char buf '"';
...
*)
and char_literal (startp:Lexing.position) buf =
  parse
  | "''" {
      Buffer.add_char buf '\'';
