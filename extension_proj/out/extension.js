...
async function dumpCstUi(output) {
    if (!client) {
        vscode.window.showWarningMessage("Jovial LSP is not running.");
        return;
    }
...
async function executeServerCommand(command, args) {
    if (!client) {
        throw new Error("Jovial LSP client is not running.");
    }
    const params = { command, arguments: args };
    return client.sendRequest(node_1.ExecuteCommandRequest.type, params);
}
function toDisplayText(res, fallback) {
...
function syntaxTreeHtml(fileLabel, uri, astText, cstText, activeTab) {
...
async function showSyntaxTreesUi(output) {
    if (!client) {
        vscode.window.showWarningMessage("Jovial LSP is not running.");
        return;
    }
...
async function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("jovial.dumpCstUi", async () => {
        try {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("jovial.showSyntaxTrees", async () => {
        try {
