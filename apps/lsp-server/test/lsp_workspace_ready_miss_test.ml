...
let wait_for_notification ~(srv : Lsp_test_helpers.server_proc)
    ~(method_ : string) ~(timeout_s : float) : Yojson.Safe.t =
  ...
  let rec loop () =
    ...
    if chunk <= 0.0 then failf "timed out waiting for %s notification" method_;
    try
...
let () =
    ...
    float_of_int
      (max 1
         (Lsp_test_helpers.getenv_int
            "JOVIAL_TEST_WORKSPACE_READY_MISS_HARD_TIMEOUT_S" ~default:180))
  in
  let workspace_files =
    max 50
      (Lsp_test_helpers.getenv_int "JOVIAL_TEST_WORKSPACE_READY_MISS_FILES"
         ~default:900)
  in
  let miss_timeout_s =
    max 2.0
  ...
  let ready_timeout_s =
    max 5.0
      (match
         Sys.getenv_opt "JOVIAL_TEST_WORKSPACE_READY_MISS_READY_TIMEOUT_S"
       with
      | None -> 90.0
  ...
  Lsp_test_helpers.with_server
    ...
    (fun srv ->
      ...
      let _miss_params_opt =
        try
          let miss_params =
            wait_for_notification ~srv ~method_:"jovial/workspaceStartupMiss"
              ~timeout_s:(min miss_timeout_s (remaining_budget ()))
          in
          (match parse_stage miss_params with
          | Some "diagHoverReady" | Some "fullyNavigable" -> ()
          | Some other ->
              failf "workspaceStartupMiss payload has unexpected stage: %s"
                other
          | None -> failf "workspaceStartupMiss payload missing stage");
          let miss_elapsed, miss_target =
            parse_elapsed_and_target miss_params
          in
      ...
      (try
         let ready_params =
           wait_for_notification ~srv ~method_:"jovial/workspaceReady"
         ...
         | Some true ->
             failf
               "workspaceReady unexpectedly reported readyWithinTarget=true in \
                miss regression"
         | None -> failf "workspaceReady payload missing readyWithinTarget"
       with Failure _ ->
         print_endline
           "lsp_workspace_ready_miss_test: note - workspaceReady not observed \
            within timeout under current constraints");
