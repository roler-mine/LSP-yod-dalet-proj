...
let sighelp_params ~(uri:string) ~(line:int) ~(character:int) : Yojson.Safe.t =
  `Assoc [
...
let () =
  ...
  in
  let timeout_s =
    float_of_int (max 1 (Lsp_test_helpers.getenv_int "JOVIAL_TEST_SIGNATURE_HELP_TIMEOUT_S" ~default:10))
  in
  ...
  let ensure_budget (phase:string) : unit =
    if remaining_budget () <= 0.0 then
      failf "signatureHelp test exceeded hard timeout (%.1fs) at %s" hard_timeout_s phase
  in
  ...
  let line, col = Lsp_test_helpers.line_col_of_first main_text ~needle:"ADD(1, 2);" in
  ...
  Lsp_test_helpers.with_server ~server_path (fun srv ->
    ...
    (match signature_result resp with
     | Some (active_param, sig_count) ->
         if sig_count <= 0 then
           failf "signatureHelp returned no signatures: %s" (Yojson.Safe.to_string resp);
         if active_param <> 1 then
           failf "signatureHelp expected activeParameter=1, got %d" active_param
     | None ->
         failf "signatureHelp returned invalid/errored response: %s" (Yojson.Safe.to_string resp));
