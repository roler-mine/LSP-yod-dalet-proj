...
let gc_snapshot () : string =
  ...
  Printf.sprintf "live_words=%d heap_words=%d top_heap_words=%d" s.Gc.live_words
...
let () =
  ...
  let ensure_budget (phase : string) : unit =
    if hard_timeout_s -. (Unix.gettimeofday () -. started) <= 0.0 then
      failf "pressure-mode transition test exceeded hard timeout (%.1fs) at %s"
  ...
  | WB.PressureNormal ->
      failf
        "expected pressure mode to move away from normal after allocation; \
         liveMB=%d (%s)"
  ...
  if not (wait_for_normal 120) then
    failf
      "pressure mode did not return to normal after release (mode=%s, \
       liveMB=%d)"
