...
let get_assoc_field (obj : Yojson.Safe.t) (key : string) : Yojson.Safe.t option
    =
  match obj with `Assoc fields -> List.assoc_opt key fields | _ -> None
...
let server_inbox_stats (resp : Yojson.Safe.t) : int * int =
  ...
  | Some result -> (
      ...
      | Some (`Assoc server_fields) ->
          ...
          in
          (max_items, max_seen)
      | _ -> failf "debugReport result is missing server inbox stats")
  | _ -> failf "debugReport response missing result field"
...
let didchange_full_text_params ~(uri : string) ~(version : int) ~(text : string)
    : Yojson.Safe.t =
  `Assoc
    [
      ( "textDocument",
...
let () =
  ...
  let timeout_s =
    float_of_int
      (max 2
         (Lsp_test_helpers.getenv_int "JOVIAL_TEST_INBOX_BP_TIMEOUT_S"
            ~default:20))
  in
  ...
  let ensure_budget (phase : string) : unit =
    if remaining_budget () <= 0.0 then
      failf "inbox backpressure test exceeded hard timeout (%.1fs) at %s"
  ...
  let line, col =
    Lsp_test_helpers.line_col_of_first text_a ~needle:"X = X + 1;"
  in
  let x_char = col + 1 in
  ...
  Lsp_test_helpers.with_server
    ...
    (fun srv ->
      ...
      let report_resp, _ =
        Lsp_test_helpers.request_timed srv ~id:700
      ...
      if max_items <= 0 then
        failf "invalid inbox max items from debugReport: %d" max_items;
      if max_seen > max_items then
        failf "inbox depth watermark exceeded cap: seen=%d cap=%d" max_seen
          max_items;
      ensure_budget "after debug report";

      let def_resp, _ =
        Lsp_test_helpers.request_timed srv ~id:701
