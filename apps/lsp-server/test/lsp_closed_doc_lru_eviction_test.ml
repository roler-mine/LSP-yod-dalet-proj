...
let () =
  ...
  let ensure_budget (phase:string) : unit =
    if hard_timeout_s -. (Unix.gettimeofday () -. started) <= 0.0 then
      failf "closed-doc LRU test exceeded hard timeout (%.1fs) at %s" hard_timeout_s phase
  in
  ...
  if closed_cache_count > lru_cap then
    failf
      "closed-doc cache exceeded LRU cap: files=%d cap=%d"
      closed_cache_count
      lru_cap;
