...
let result_assoc (resp : Yojson.Safe.t) : (string * Yojson.Safe.t) list =
  ...
  | `Assoc fields -> (
      ...
      | Some (`Assoc rf) -> rf
      | _ ->
          failf "workspace/executeCommand result was not an object: %s"
            (Yojson.Safe.to_string resp))
  | _ -> failf "response is not an object: %s" (Yojson.Safe.to_string resp)

let int_field_default (fields : (string * Yojson.Safe.t) list) ~(name : string)
    ~(default : int) : int =
...
let bool_field_default (fields : (string * Yojson.Safe.t) list) ~(name : string)
    ~(default : bool) : bool =
  match List.assoc_opt name fields with Some (`Bool b) -> b | _ -> default

let find_symbol_id_by_key (fields : (string * Yojson.Safe.t) list)
      ...
      |> List.find_map (function
        | `Assoc sf -> (
            ...
            | Some (`String k), Some (`String id)
              when String.uppercase_ascii (String.trim k) = key
                   && String.trim id <> "" ->
                Some id
            | _ -> None)
        | _ -> None)
...
let did_change_replace_params ~(uri : string) ~(version : int)
    ~(old_text : string) ~(new_text : string) : Yojson.Safe.t =
              ...
              [
                ( "range",
                  `Assoc
                    [
                      ( "start",
                        `Assoc [ ("line", `Int 0); ("character", `Int 0) ] );
                      ( "end",
                        `Assoc
                          [
                            ("line", `Int end_line); ("character", `Int end_char);
                          ] );
                    ] );
...
let upsert_has_key (fields : (string * Yojson.Safe.t) list) ~(key : string) :
    bool =
  let key = String.uppercase_ascii key in
  ...
  | Some (`List xs) ->
      ...
      |> List.exists (function
        | `Assoc sf -> (
            match (List.assoc_opt "key" sf, List.assoc_opt "id" sf) with
            | Some (`String k), Some (`String id) ->
                String.uppercase_ascii (String.trim k) = key
                && String.trim id <> ""
            | _ -> false)
        | _ -> false)
  | _ -> false

let rec poll_for_delta ~(srv : Lsp_test_helpers.server_proc) ~(doc_uri : string)
    ~(base_revision : int) ~(expect_delete_id : string) ~(expect_key : string)
    ~(timeout_s : float) ~(deadline : float) ~(next_id : int)
    ~(last_payload : string option) : unit =
  if Unix.gettimeofday () >= deadline then
    failf "lsif delta by-id did not converge before timeout; last payload=%s"
      (match last_payload with Some s -> s | None -> "<none>");
  let resp, _ =
    Lsp_test_helpers.request_timed srv ~id:next_id
      ~method_:"workspace/executeCommand"
      ~params:
        (exec_params ~command:"jovial.dumpLsifDelta"
           ~arguments:[ `String doc_uri; `Int base_revision ])
      ~timeout_s
  in
  ...
  let has_expected_delete =
    List.exists (fun x -> x = expect_delete_id) deletes
  in
  let has_expected_upsert = upsert_has_key fields ~key:expect_key in
  if (not reset) && has_expected_delete && has_expected_upsert then ()
  else (
    Thread.delay 0.15;
    poll_for_delta ~srv ~doc_uri ~base_revision ~expect_delete_id ~expect_key
      ~timeout_s ~deadline ~next_id:(next_id + 1) ~last_payload:(Some payload_s))
...
let () =
  ...
  let timeout_s =
    float_of_int
      (max 1
         (Lsp_test_helpers.getenv_int "JOVIAL_TEST_LSIF_DELTA_ID_TIMEOUT_S"
            ~default:10))
  in
  ...
  let ensure_budget (phase : string) : unit =
    if remaining_budget () <= 0.0 then
      failf "lsif delta by-id test exceeded hard timeout (%.1fs) at %s"
  ...
  let before_line, before_col =
    Lsp_test_helpers.line_col_of_first before_text ~needle:"VALUE = VALUE + 1;"
  in
  let after_line, after_col =
    Lsp_test_helpers.line_col_of_first after_text ~needle:"VALUE2 = VALUE2 + 1;"
  in
  ...
  Lsp_test_helpers.with_server ~server_path (fun srv ->
      ...
      let idx_fields =
        poll_index 2
          (Unix.gettimeofday () +. min 4.0 (max 1.0 (remaining_budget ())))
      in
      let base_revision =
        int_field_default idx_fields ~name:"revision" ~default:(-1)
      in
      ...
      Lsp_test_helpers.send_notification srv ~method_:"textDocument/didChange"
        ~params:
          (did_change_replace_params ~uri:doc_uri ~version:2
             ~old_text:before_text ~new_text:after_text);

      ignore
        (Lsp_test_helpers.request_timed srv ~id:21
           ~method_:"textDocument/definition"
           ~params:
             (Lsp_test_helpers.definition_request_params ~uri:doc_uri
                ~line:after_line ~character:(after_col + 1))
           ~timeout_s:(min timeout_s (remaining_budget ())));

      ensure_budget "after didChange";
      poll_for_delta ~srv ~doc_uri ~base_revision ~expect_delete_id:old_value_id
        ~expect_key:"VALUE2"
        ~timeout_s:(min timeout_s (remaining_budget ()))
        ~deadline:
          (Unix.gettimeofday () +. min 4.0 (max 1.0 (remaining_budget ())))
        ~next_id:3 ~last_payload:None;
