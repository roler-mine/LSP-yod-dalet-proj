...
let () =
  ...
  if
    not
      (List.exists (function `DocumentSymbol _ -> true | _ -> false) symbols)
  then failf "expected typed document symbols";

  let completion_pos =
    find_substring source_text ~needle:"RESULT = " |> fun off ->
    position_of_offset source_text (off + String.length "RESULT = ")
  in
  let completions =
    Lib.Workspace.completion_items_for ws ~uri ~pos:completion_pos
  in
  if
    not
      (List.exists
         (fun item -> item.T.CompletionItem.label = "ADD")
         completions)
  then failf "expected ADD completion item";

  let range =
    let start =
      position_of_offset source_text
        (find_substring source_text ~needle:"ADD(1, 2)")
    in
  ...
  let labels =
    hints
    |> List.map (fun hint ->
        match hint.T.InlayHint.label with `String s -> s | `List _ -> "")
  in
