...
let wait_for_workspace_ready_stage ~(srv : Lsp_test_helpers.server_proc)
    ~(stage : ready_stage) ~(timeout_s : float) : Yojson.Safe.t =
  let wanted = stage_wire_name stage in
  let deadline = Unix.gettimeofday () +. timeout_s in
  let rec loop () =
    let remaining = deadline -. Unix.gettimeofday () in
    if remaining <= 0.0 then
      failf "timed out waiting for jovial/workspaceReady stage=%s notification"
        wanted;
...
let assert_stage_ready_payload ~(stage : ready_stage) (params : Yojson.Safe.t) :
    unit =
  ...
  | `Assoc fields -> (
      ...
      | Some (`Assoc readiness_fields) -> (
          ...
          if elapsed_ms < 0 then
            failf "workspaceReady payload missing readiness.stages.%s.elapsedMs"
              stage_key;
          let is_ready =
          ...
          if not is_ready then
            failf
              "workspaceReady payload reported \
               readiness.stages.%s.isReady=false"
              stage_key;
          match stage with
          | StageFullyNavigable ->
              let top_ready =
              ...
              if not top_ready then
                failf
                  "workspaceReady fullyNavigable payload reported \
                   readiness.isReady=false"
          | StageDiagHoverReady -> ())
      | _ -> failf "workspaceReady payload missing readiness object")
  | _ -> failf "workspaceReady payload is not an object"
    ...
    float_of_int
      (max 1
         (Lsp_test_helpers.getenv_int
            "JOVIAL_TEST_WORKSPACE_READY_HARD_TIMEOUT_S" ~default:120))
  in
  let notif_timeout_s =
    max 0.5
      (match Sys.getenv_opt "JOVIAL_TEST_WORKSPACE_READY_TIMEOUT_S" with
      | None -> 8.0
      | Some raw -> ( try float_of_string (String.trim raw) with _ -> 8.0))
  in
