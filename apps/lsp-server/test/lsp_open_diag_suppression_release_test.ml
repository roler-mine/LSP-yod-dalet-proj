...
let () =
    ...
    float_of_int
      (max 1
         (Lsp_test_helpers.getenv_int
            "JOVIAL_TEST_OPEN_DIAG_SUPPRESS_HARD_TIMEOUT_S"
            ~default:180))
  in
  ...
  let root = Lsp_test_helpers.mk_temp_dir "jovial-lsp-open-diag-suppress-release" in
  ...
  Lsp_test_helpers.with_server
    ...
    (fun srv ->
      ...
      if List.exists (fun m -> contains_substring ~haystack:m ~needle:"MISSING") first_msgs then
        failf "unexpected unresolved cross-module diagnostic before readiness release";

      ensure_budget "wait diagHoverReady";
      wait_for_workspace_ready_stage ~srv ~stage:"diagHoverReady" ~timeout_s:25.0;

      ensure_budget "metric suppression/ready transition";
      wait_for_metric_calls
        ~srv
        ~name:"diag.xmodule_ready_transition"
        ~min_calls:1
        ~timeout_s:8.0;
      wait_for_metric_calls
        ~srv
