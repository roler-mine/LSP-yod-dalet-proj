...
let workspace_files ~(root : string) ~(suffix : string) :
...
let () =
  ...
  let timeout_s =
    float_of_int
      (max 1
         (Lsp_test_helpers.getenv_int "JOVIAL_TEST_MULTI_ROOT_TIMEOUT_S"
            ~default:10))
  in
  ...
  let ensure_budget (phase : string) : unit =
    if remaining_budget () <= 0.0 then
      failf "multi-root routing test exceeded hard timeout (%.1fs) at %s"
  ...
  let _a_main_path, _a_main_text, _a_target_path =
    workspace_files ~root:root_a ~suffix:"A"
  in
  let b_main_path, b_main_text, b_target_path =
    workspace_files ~root:root_b ~suffix:"B"
  in
  ...
  let line, col =
    Lsp_test_helpers.line_col_of_first b_main_text
      ~needle:"TARGET'VAL_B = TARGET'VAL_B + 1;"
  in
  ...
  Lsp_test_helpers.with_server ~server_path (fun srv ->
      ...
      in
      Lsp_test_helpers.send_request srv ~id:1 ~method_:"initialize"
        ~params:init_params;
      let init_resp =
        Lsp_test_helpers.wait_for_response srv ~id:1
      ...
      let def_resp, _ =
        Lsp_test_helpers.request_timed srv ~id:2
          ...
          ~params:
            (Lsp_test_helpers.definition_request_params ~uri:b_main_uri ~line
               ~character:def_char)
          ~timeout_s:(min timeout_s (remaining_budget ()))
      in
      if not (response_has_result_uri def_resp ~uri:b_target_uri) then
        failf "expected definition to resolve in root-B target (%s), got: %s"
          b_target_uri
