...
let create_workspace ~(root : string) ~(file_count : int) :
    string * string * string =
  for i = 0 to file_count - 1 do
    let path = Filename.concat root (Printf.sprintf "BIG_%04d.jov" i) in
    let prev_call =
      if i <= 0 then "" else Printf.sprintf "  PROC_%04d(OUTP:OUTP);\n" (i - 1)
    in
    let text =
      String.concat "\n"
        [
  ...
  let open_path =
    Filename.concat root (Printf.sprintf "BIG_%04d.jov" open_idx)
  in
  let target_path =
    Filename.concat root (Printf.sprintf "BIG_%04d.jov" (open_idx - 1))
  in
  let open_text =
    try
      ...
      close_in_noerr ic;
      s
    with _ -> failf "failed reading generated open test file: %s" open_path
  in
...
let () =
  ...
  let file_count =
    max 3
      (Lsp_test_helpers.getenv_int "JOVIAL_TEST_UNSCOPED_XFILE_COUNT"
         ~default:120)
  in
  let timeout_s =
    float_of_int
      (max 1
         (Lsp_test_helpers.getenv_int "JOVIAL_TEST_UNSCOPED_XFILE_TIMEOUT_S"
            ~default:10))
  in
  let first_max_ms =
    float_of_int
      (max 1
         (Lsp_test_helpers.getenv_int "JOVIAL_TEST_UNSCOPED_XFILE_FIRST_MAX_MS"
            ~default:2000))
  in
    ...
    float_of_int
      (max 1
         (Lsp_test_helpers.getenv_int
            "JOVIAL_TEST_UNSCOPED_XFILE_SETTLE_TIMEOUT_S" ~default:6))
  in
  ...
  let ensure_budget (phase : string) : unit =
    ...
    if left <= 0.0 then
      failf
        "unscoped cross-file definition test exceeded hard timeout (%.1fs) at \
         %s"
  ...
  let needle = Printf.sprintf "PROC_%04d(OUTP:OUTP);" (file_count - 2) in
  ...
  Lsp_test_helpers.with_server ~server_path (fun srv ->
      ...
      (if not (response_has_result_uri resp ~uri:target_uri) then
         let deadline =
           Unix.gettimeofday ()
           +. min settle_timeout_s (max 1.0 (remaining_budget ()))
         in
         let rec wait_for_target next_id last_payload =
           if Unix.gettimeofday () >= deadline then
             failf
               "first unscoped cross-file definition did not converge to \
                expected target %s within %.1fs; last payload=%s"
               target_uri settle_timeout_s last_payload;
           let retry_resp, _ =
