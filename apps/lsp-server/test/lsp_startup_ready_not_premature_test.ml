...
let startup_state_of_report (report_fields:(string * Yojson.Safe.t) list) : startup_state =
  let startup =
...
let request_debug_startup_state
    ~(srv:Lsp_test_helpers.server_proc)
    ~(id:int)
    ~(uri:string)
    ~(timeout_s:float)
...
let wait_for_workspace_ready
  ...
  let rec loop () =
    ...
    if remaining <= 0.0 then
      failf "timed out waiting for jovial/workspaceReady";
    match wait_for_notification_opt ~srv ~method_:"jovial/workspaceReady" ~timeout_s:(min 0.4 remaining) with
    ...
    float_of_int
      (max 1
         (Lsp_test_helpers.getenv_int
            "JOVIAL_TEST_READY_NOT_PREMATURE_HARD_TIMEOUT_S"
            ~default:180))
  in
  let short_guard_timeout_s =
    max 0.2
      (match Sys.getenv_opt "JOVIAL_TEST_READY_NOT_PREMATURE_GUARD_TIMEOUT_S" with
       | None -> 1.0
       | Some raw -> (try float_of_string (String.trim raw) with _ -> 1.0))
  in
  let ready_timeout_s =
    max 3.0
      (match Sys.getenv_opt "JOVIAL_TEST_READY_NOT_PREMATURE_READY_TIMEOUT_S" with
       | None -> 25.0
  ...
  Lsp_test_helpers.with_server
    ...
    (fun srv ->
      ...
      (match maybe_early_ready with
       ...
       | Some params ->
           ...
           if st.index_reconcile_pending then
             failf "received premature workspaceReady while index reconcile was still pending";
      ...
      if final_state.open_docs_pending_parse <> 0 then
        failf
          "final workspaceReady payload has openDocsPendingParse=%d"
