...
let assert_index_contains_only_root_symbol
    ~(index_fields : (string * Yojson.Safe.t) list) ~(must_have : string)
    ~(must_not_have : string) : unit =
...
let () =
  ...
  let timeout_s =
    float_of_int
      (max 1
         (Lsp_test_helpers.getenv_int "JOVIAL_TEST_LSIF_ROOT_TIMEOUT_S"
            ~default:10))
  in
  ...
  let ensure_budget (phase : string) : unit =
    if remaining_budget () <= 0.0 then
      failf "lsif root routing test exceeded hard timeout (%.1fs) at %s"
  ...
  Lsp_test_helpers.with_server ~server_path (fun srv ->
          ...
          [
            ...
            ( "workspaceFolders",
              `List
                [
                  `Assoc
                    [ ("uri", `String root_a_uri); ("name", `String "rootA") ];
                  `Assoc
                    [ ("uri", `String root_b_uri); ("name", `String "rootB") ];
                ] );
            ("capabilities", `Assoc []);
          ]
      in
      ...
      let rec poll_index ~uri next_id deadline =
        if Unix.gettimeofday () >= deadline then
          failf
            "lsif root routing test timed out waiting for non-empty index (%s)"
            uri;
        let resp, _ =
      ...
      assert_index_contains_only_root_symbol ~index_fields:idx_a_fields
        ~must_have:"ALPHA_ROOT" ~must_not_have:"BETA_ROOT";
      assert_index_contains_only_root_symbol ~index_fields:idx_b_fields
        ~must_have:"BETA_ROOT" ~must_not_have:"ALPHA_ROOT";

      let rev_a =
        int_field_default idx_a_fields ~name:"revision" ~default:(-1)
      in
      let rev_b =
        int_field_default idx_b_fields ~name:"revision" ~default:(-1)
      in
      if rev_a < 0 || rev_b < 0 then
        failf "lsif index revision missing in root routing test";
        ...
        Lsp_test_helpers.request_timed srv ~id:next_id
          ...
          ~params:
            (exec_params ~command:"jovial.dumpLsifDelta"
               ~arguments:[ `String a_uri; `Int rev_a ])
          ~timeout_s:(min timeout_s (remaining_budget ()))
      in
      let delta_b_resp, _ =
        Lsp_test_helpers.request_timed srv ~id:(next_id + 1)
          ~method_:"workspace/executeCommand"
          ~params:
            (exec_params ~command:"jovial.dumpLsifDelta"
               ~arguments:[ `String b_uri; `Int rev_b ])
          ~timeout_s:(min timeout_s (remaining_budget ()))
      in
