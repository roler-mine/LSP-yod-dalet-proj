...
let () =
  ...
  in
  let timeout_s =
    float_of_int (max 2 (Lsp_test_helpers.getenv_int "JOVIAL_TEST_IO_SIZE_TIMEOUT_S" ~default:12))
  in
  ...
  let ensure_budget (phase:string) : unit =
    if remaining_budget () <= 0.0 then
      failf "io size-limit test exceeded hard timeout (%.1fs) at %s" hard_timeout_s phase
  in
