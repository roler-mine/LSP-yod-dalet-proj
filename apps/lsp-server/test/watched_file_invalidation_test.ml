...
let () =
  let tmp_root =
    Filename.concat
      ...
      "jovial-watch-invalidation-test"
  in
  ...
  Lib.Workspace.apply_watched_file_changes ws ~changes:[ (src_path, `Changed) ];
  ...
  | _ ->
      failf
        "lsif delta response should be object after watched-file invalidation");
