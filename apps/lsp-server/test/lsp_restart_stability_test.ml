...
let run_cycle
  ...
  Lsp_test_helpers.with_server ~server_path (fun srv ->
    ignore
      (Lsp_test_helpers.initialize_and_open srv ~root_uri ~doc_uri ~doc_text:main_text ~timeout_s);
    ...
    if graceful then (
      ...
      | Unix.WEXITED 0 -> ()
      | st -> failf "cycle %d graceful restart exited with %s" cycle (match st with
...
let () =
  ...
  in
  let hard_timeout_s =
    float_of_int (max 1 (Lsp_test_helpers.getenv_int "JOVIAL_TEST_RESTART_HARD_TIMEOUT_S" ~default:300))
  in
  let cycles = max 1 (Lsp_test_helpers.getenv_int "JOVIAL_TEST_RESTART_CYCLES" ~default:14) in
  let timeout_s =
    float_of_int (max 1 (Lsp_test_helpers.getenv_int "JOVIAL_TEST_RESTART_TIMEOUT_S" ~default:8))
  in
  ...
  for i = 1 to cycles do
    ...
    if left <= 0.0 then
      failf "restart stability test exceeded hard timeout (%.1fs) at cycle %d" hard_timeout_s i;
    let cycle_timeout = min timeout_s left in
    if cycle_timeout <= 0.0 then
      failf "restart stability test has no remaining timeout budget at cycle %d" i;
