...
type completion_item = { label : string; sort_text : string option }
...
let () =
  ...
  let timeout_s =
    float_of_int
      (max 1
         (Lsp_test_helpers.getenv_int "JOVIAL_TEST_COMPLETION_RANK_TIMEOUT_S"
            ~default:10))
  in
  ...
  let ensure_budget (phase : string) : unit =
    if remaining_budget () <= 0.0 then
      failf "completion ranking test exceeded hard timeout (%.1fs) at %s"
  ...
  let line, col =
    Lsp_test_helpers.line_col_of_first main_text ~needle:"LOCALX = LO;"
  in
  ...
  Lsp_test_helpers.with_server ~server_path (fun srv ->
      ...
      let find_item label =
        items
        |> List.find_map (fun it ->
            if String.uppercase_ascii it.label = label then Some it else None)
      in
      let local =
      ...
      let idx_of label =
        let rec loop i = function
          | [] -> None
          | it :: tl ->
              if String.uppercase_ascii it.label = label then Some i
              else loop (i + 1) tl
        in
        loop 0 items
      in
      let local_idx =
        match idx_of "LOCALX" with Some i -> i | None -> max_int
      in
      let loc_idx = match idx_of "LOC" with Some i -> i | None -> max_int in
      if not (local_idx < loc_idx) then
        failf "expected LOCALX to rank before LOC (idx %d vs %d)" local_idx
          loc_idx;
