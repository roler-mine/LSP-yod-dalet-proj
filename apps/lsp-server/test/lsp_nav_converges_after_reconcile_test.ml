...
let replace_workspace_with_fresh_targets
    ~(root:string)
    ~(old_pool:string)
...
let () =
    ...
    float_of_int
      (max 1
         (Lsp_test_helpers.getenv_int
            "JOVIAL_TEST_NAV_RECONCILE_HARD_TIMEOUT_S"
            ~default:180))
  in
  let first_max_ms =
    float_of_int
      (max 1
         (Lsp_test_helpers.getenv_int
            "JOVIAL_TEST_NAV_RECONCILE_FIRST_MAX_MS"
            ~default:2000))
  in
  ...
  let ensure_budget (phase:string) =
    if remaining_budget () <= 0.0 then
      failf
        "nav convergence after reconcile test exceeded hard timeout (%.1fs) at %s"
        hard_timeout_s
        phase
  in
  ...
  let line, col =
    Lsp_test_helpers.line_col_of_first main_text ~needle:"FRESH'VAL = FRESH'VAL + 1;"
  in
  ...
  Lsp_test_helpers.with_server
    ...
    (fun srv ->
      ...
      if not (debug_checkpoint_loaded ~srv ~id:2 ~uri:main_uri ~timeout_s:2.0) then
        failf "expected checkpointLoaded=true on stale-index run";
      ...
      if first_ms > first_max_ms then
        failf
          "first definition response was too slow: %.1fms (limit %.1fms)"
          first_ms
          first_max_ms;
      ...
      if not (response_has_result_uri first_resp ~uri:target_uri) then (
        ...
        let rec settle req_id =
          if Unix.gettimeofday () >= deadline then
            failf
              "definition did not converge to %s within %.1fs after reconcile scheduling"
              target_uri
              settle_timeout_s;
