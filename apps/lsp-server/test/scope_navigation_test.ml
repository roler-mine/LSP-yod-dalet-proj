...
let () =
  ...
  let pos_inp_usage_in_bar =
    find_nth_substring source_text ~needle:"INP" ~nth:3 |> fun off ->
    position_of_offset source_text (off + 1)
  in
  let pos_v_usage_in_bar =
    find_nth_substring source_text ~needle:"V" ~nth:3 |> fun off ->
    position_of_offset source_text (off + 1)
  in

  let def_inp =
    Lib.Workspace.definition_locations_for ws ~uri ~pos:pos_inp_usage_in_bar
  in
  let def_v =
    Lib.Workspace.definition_locations_for ws ~uri ~pos:pos_v_usage_in_bar
  in

  expect_eq_int ~name:"BAR input parameter resolves in BAR scope"
    ~got:(line_of_first_definition_location def_inp)
    ~want:6;
  expect_eq_int ~name:"BAR local item resolves in BAR scope"
