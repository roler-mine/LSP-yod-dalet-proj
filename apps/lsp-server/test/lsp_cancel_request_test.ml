...
let () =
  ...
  in
  let lines =
    max 20000 (Lsp_test_helpers.getenv_int "JOVIAL_TEST_CANCEL_LINES" ~default:80000)
  in
  let timeout_s =
    float_of_int (max 2 (Lsp_test_helpers.getenv_int "JOVIAL_TEST_CANCEL_TIMEOUT_S" ~default:12))
  in
  ...
  let ensure_budget (phase:string) : unit =
    if remaining_budget () <= 0.0 then
      failf "cancel-request test exceeded hard timeout (%.1fs) at %s" hard_timeout_s phase
  in
  ...
  Lsp_test_helpers.with_server ~server_path (fun srv ->
    ...
    Lsp_test_helpers.send_request srv
      ~id:2
    ...
    Lsp_test_helpers.send_notification srv
      ~method_:"$/cancelRequest"
      ~params:(`Assoc [ "id", `Int 2 ]);

    let resp =
      Lsp_test_helpers.wait_for_response srv
        ~id:2
        ~timeout_s:(min timeout_s (remaining_budget ()))
    in
