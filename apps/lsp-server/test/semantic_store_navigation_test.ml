...
let () =
  ...
  let pos_inp_usage =
    find_nth_substring source_text ~needle:"INP" ~nth:1
    |> fun off -> position_of_offset source_text (off + 1)
  in

  let def1 = Lib.Workspace.definition_locations_for ws ~uri ~pos:pos_inp_usage in
  let def2 = Lib.Workspace.definition_locations_for ws ~uri ~pos:pos_inp_usage in
  expect_eq_int
    ~name:"definition line first call"
    ~got:(line_of_first_definition_location def1)
    ~want:1;
  expect_eq_int
    ~name:"definition line second call"
