...
let expect_rename_null (resp : Yojson.Safe.t) : unit =
  ...
  | Some `Null -> ()
  | Some v ->
      failf "rename expected null under soft budget exceed, got: %s"
        (Yojson.Safe.to_string v)
  | None -> failf "rename response missing result field"
...
let debug_perf_params : Yojson.Safe.t =
  `Assoc
    [ ("command", `String "jovial.debugPerfStats"); ("arguments", `List []) ]
...
let () =
  ...
  let lines =
    max 5000
      (Lsp_test_helpers.getenv_int "JOVIAL_TEST_NAV_SOFT_BUDGET_LINES"
         ~default:30000)
  in
  let timeout_s =
    float_of_int
      (max 1
         (Lsp_test_helpers.getenv_int "JOVIAL_TEST_NAV_SOFT_BUDGET_TIMEOUT_S"
            ~default:20))
  in
  ...
  let ensure_budget (phase : string) : unit =
    ...
    if left <= 0.0 then
      failf "nav soft-budget test exceeded hard timeout (%.1fs) at %s"
    ...
    (fun srv ->
      ...
      let completion_resp, _ =
        Lsp_test_helpers.request_timed srv ~id:4
          ...
          ~params:(completion_params ~uri:doc_uri ~line ~character:x_char)
          ~timeout_s:(min timeout_s (remaining_budget ()))
      in
      ...
      let rename_resp, _ =
        Lsp_test_helpers.request_timed srv ~id:5 ~method_:"textDocument/rename"
          ~params:
            (rename_params ~uri:doc_uri ~line ~character:x_char
               ~new_name:"X_RENAMED")
          ~timeout_s:(min timeout_s (remaining_budget ()))
      in
      ...
      let perf_resp, _ =
        Lsp_test_helpers.request_timed srv ~id:6
          ~method_:"workspace/executeCommand" ~params:debug_perf_params
          ~timeout_s:(min timeout_s (remaining_budget ()))
      in
      ensure_no_error ~method_:"workspace/executeCommand" perf_resp;
      let soft_budget_calls =
        metric_calls perf_resp ~name:"nav.soft_budget_exceeded"
      in
      if soft_budget_calls <= 0 then
        failf "expected nav.soft_budget_exceeded metric calls > 0, got %d"
          soft_budget_calls;
