          ...
          List.exists
            (function
              | `Assoc lf -> (
                  ...
                  | Some (`String u) ->
                      Lsp_test_helpers.normalize_uri_for_compare u = target
                  | _ -> false)
              | _ -> false)
            xs
      | Some (`Assoc lf) -> (
...
let request_hover ~(srv : Lsp_test_helpers.server_proc) ~(id : int)
    ~(uri : string) ~(line : int) ~(character : int) ~(timeout_s : float) :
    Yojson.Safe.t * float =
  Lsp_test_helpers.request_timed srv ~id ~method_:"textDocument/hover"
...
let () =
  ...
  let timeout_s =
    float_of_int
      (max 1
         (Lsp_test_helpers.getenv_int "JOVIAL_TEST_REF_PROC_NAV_TIMEOUT_S"
            ~default:10))
  in
    ...
    float_of_int
      (max 1
         (Lsp_test_helpers.getenv_int
            "JOVIAL_TEST_REF_PROC_NAV_SETTLE_TIMEOUT_S" ~default:8))
  in
  ...
  let ensure_budget (phase : string) : unit =
    if remaining_budget () <= 0.0 then
      failf "ref-proc nav test exceeded hard timeout (%.1fs) at %s"
  ...
  let lib_decl_line0, _ =
    Lsp_test_helpers.line_col_of_first lib_text ~needle:"DEF PROC HELPER RENT;"
  in
  let ref_decl_line0, _ =
    Lsp_test_helpers.line_col_of_first main_text ~needle:"REF PROC HELPER RENT;"
  in
  ...
  Lsp_test_helpers.with_server ~server_path (fun srv ->
      ...
      let def_deadline =
        Unix.gettimeofday () +. min settle_timeout_s (remaining_budget ())
      in
      let rec wait_for_definition req_id last_payload =
        if Unix.gettimeofday () >= def_deadline then
          failf
            "definition did not converge to implementation %s within %.1fs; \
             last payload=%s"
            lib_uri settle_timeout_s last_payload;
        let resp, _ =
          request_definition ~srv ~id:req_id ~uri:doc_uri ~line:call_line
      ...
      let rec wait_for_hover req_id last_payload =
        if Unix.gettimeofday () >= hover_deadline then
          failf
            "hover did not converge to implementation location %s within \
             %.1fs; last payload=%s"
            expected_hover_loc settle_timeout_s last_payload;
        let resp, _ =
          request_hover ~srv ~id:req_id ~uri:doc_uri ~line:call_line
            ~character:hover_char
            ~timeout_s:(min timeout_s (remaining_budget ()))
        in
        if response_has_error resp then
