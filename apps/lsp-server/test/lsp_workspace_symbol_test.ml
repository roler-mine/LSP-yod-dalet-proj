...
let () =
  ...
  let timeout_s =
    float_of_int
      (max 1
         (Lsp_test_helpers.getenv_int "JOVIAL_TEST_WORKSPACE_SYMBOL_TIMEOUT_S"
            ~default:10))
  in
  ...
  let ensure_budget (phase : string) : unit =
    if remaining_budget () <= 0.0 then
      failf "workspace/symbol test exceeded hard timeout (%.1fs) at %s"
  ...
  Lsp_test_helpers.with_server ~server_path (fun srv ->
      ...
      let resp, _ =
        Lsp_test_helpers.request_timed srv ~id:2 ~method_:"workspace/symbol"
      ...
      if not (List.exists (fun n -> String.uppercase_ascii n = "ALPHA") names)
      then
        failf "workspace/symbol missing ALPHA result: %s"
