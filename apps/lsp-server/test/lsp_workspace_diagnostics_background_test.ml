...
type diag_counts = { errors : int; warnings : int }
...
let wait_for_uri_diagnostics ~(srv : Lsp_test_helpers.server_proc)
    ~(target_uri : string) ~(timeout_s : float) : int list option =
...
let run_mode_case ~(server_path : string) ~(mode : string)
    ~(expect_errors : bool) ~(expect_warnings : bool)
    ~(enforce_no_warnings_when_false : bool) : unit =
  let root =
    Lsp_test_helpers.mk_temp_dir ("jovial-lsp-workspace-diags-" ^ mode)
  in
  ...
  Lsp_test_helpers.with_server
    ...
    (fun srv ->
      ...
      let severities_opt =
        wait_for_uri_diagnostics ~srv ~target_uri:broken_uri ~timeout_s:12.0
      in
      let counts =
      ...
      if expect_errors then (
        if counts.errors <= 0 then
          failf
            "workspace diagnostics mode=%s expected errors for unopened file, \
             got none"
            mode)
      else if counts.errors > 0 then
        failf
          "workspace diagnostics mode=%s expected no unopened-file errors, got \
           %d"
          mode counts.errors;
      ...
      if expect_warnings then (
        if counts.warnings <= 0 then
          failf
            "workspace diagnostics mode=%s expected warnings for unopened \
             file, got none"
            mode)
      else if enforce_no_warnings_when_false && counts.warnings > 0 then
        failf
          "workspace diagnostics mode=%s expected no unopened-file warnings, \
           got %d"
          mode counts.warnings;
...
let () =
  ...
  let ensure_budget (phase : string) : unit =
    ...
    if left <= 0.0 then
      failf
        "workspace diagnostics background test exceeded hard timeout (%.1fs) \
         at %s"
        hard_timeout_s phase
  in
  ensure_budget "before mode=off";
  run_mode_case ~server_path ~mode:"off" ~expect_errors:false
    ~expect_warnings:false ~enforce_no_warnings_when_false:true;
  ensure_budget "before mode=errors";
  run_mode_case ~server_path ~mode:"errors" ~expect_errors:true
    ~expect_warnings:false ~enforce_no_warnings_when_false:true;
  let expect_all_mode_warnings =
    getenv_bool "JOVIAL_TEST_REQUIRE_ALL_MODE_WARNINGS" ~default:false
  in
  ensure_budget "before mode=all";
  run_mode_case ~server_path ~mode:"all" ~expect_errors:true
