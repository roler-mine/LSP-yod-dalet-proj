...
let did_change_insert_params ~(uri : string) ~(version : int) ~(line : int)
              ...
              [
                ( "range",
                  `Assoc
                    [
                      ( "start",
                        `Assoc [ ("line", `Int line); ("character", `Int 0) ] );
                      ( "end",
                        `Assoc [ ("line", `Int line); ("character", `Int 0) ] );
                    ] );
                ("text", `String text);
...
let () =
  ...
  let hard_timeout_s =
    float_of_int
      (max 1
         (Lsp_test_helpers.getenv_int "JOVIAL_TEST_EDIT_HARD_TIMEOUT_S"
            ~default:300))
  in
  let filler_lines =
    max 2000
      (Lsp_test_helpers.getenv_int "JOVIAL_TEST_EDIT_LARGE_FILE_LINES"
         ~default:10000)
  in
  let timeout_s =
    float_of_int
      (max 1
         (Lsp_test_helpers.getenv_int "JOVIAL_TEST_EDIT_TIMEOUT_S" ~default:20))
  in
  let max_nav_after_edit_ms =
    float_of_int
      (max 1
         (Lsp_test_helpers.getenv_int "JOVIAL_TEST_EDIT_MAX_NAV_MS"
            ~default:1500))
  in
  ...
  let ensure_budget (phase : string) : unit =
    ...
    if left <= 0.0 then
      failf "didChange latency test exceeded hard timeout (%.1fs) at %s"
  ...
  Lsp_test_helpers.with_server ~server_path (fun srv ->
      ...
      let def_params =
        Lsp_test_helpers.definition_request_params ~uri:doc_uri ~line:use_line
          ~character:(use_col + 1)
      in
      let warm_resp, _ =
        Lsp_test_helpers.request_timed srv ~id:2
          ~method_:"textDocument/definition" ~params:def_params
          ~timeout_s:(min timeout_s (remaining_budget ()))
      in
      ...
      Lsp_test_helpers.send_notification srv ~method_:"textDocument/didChange"
        ~params:
          (did_change_insert_params ~uri:doc_uri ~version:2 ~line:edit_line
      ...
      let hover_resp_after, hover_ms_after =
        Lsp_test_helpers.request_timed srv ~id:4 ~method_:"textDocument/hover"
          ~params:
            (hover_params ~uri:doc_uri ~line:use_line ~character:(use_col + 1))
          ~timeout_s:(min timeout_s (remaining_budget ()))
      in
      ...
      if def_ms_after > max_nav_after_edit_ms then
        failf
          "definition after didChange too slow: %.1fms (limit %.1fms, filler \
           lines=%d)"
          def_ms_after max_nav_after_edit_ms filler_lines;

      if hover_ms_after > max_nav_after_edit_ms then
        failf
          "hover after didChange too slow: %.1fms (limit %.1fms, filler \
           lines=%d)"
          hover_ms_after max_nav_after_edit_ms filler_lines;
