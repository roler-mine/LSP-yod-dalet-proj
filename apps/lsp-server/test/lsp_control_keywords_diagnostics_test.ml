...
let assert_no_false_control_keyword_diag ~(phase:string) (diags:Yojson.Safe.t list) : unit =
  let offending =
    diags
  ...
  if offending <> [] then
    failf
      "control-keyword diagnostics regression at %s: %s"
      phase
...
let expect_empty_diags_for_open
    ~(srv:Lsp_test_helpers.server_proc)
    ~(uri:string)
    ~(timeout_s:float)
    ~(phase:string)
  ...
  if diags <> [] then
    failf
      "expected empty diagnostics for %s at %s, got %d entries"
      uri
      phase
      (List.length diags)
...
let wait_uri_diag_contains_or_fail
    ~(srv:Lsp_test_helpers.server_proc)
    ~(uri:string)
    ~(timeout_s:float)
    ~(phase:string)
  ...
  let rec loop () =
    ...
    if remaining <= 0.0 then
      failf
        "expected diagnostics for %s at %s to include %S within %.2fs"
        uri
        phase
        needle
        timeout_s;
    ...
    if chunk <= 0.0 then
      failf
        ...
        needle
        timeout_s;
    match Lsp_test_helpers.wait_for_publish_diagnostics_for_uri ~srv ~target_uri:uri ~timeout_s:chunk with
    | None ->
        loop ()
    | Some diags ->
        assert_no_false_control_keyword_diag ~phase diags;
...
let () =
  ...
  let authoritative_timeout_s =
    max open_timeout_s
      (match Sys.getenv_opt "JOVIAL_TEST_CONTROL_KEYWORDS_AUTHORITATIVE_TIMEOUT_S" with
       | None -> 10.0
       | Some raw ->
    ...
    float_of_int
      (max 1
         (Lsp_test_helpers.getenv_int
            "JOVIAL_TEST_CONTROL_KEYWORDS_HARD_TIMEOUT_S"
            ~default:120))
  in
  ...
  let exit_invalid_uri = Lsp_test_helpers.lsp_doc_uri_of_path exit_invalid_path in
  let abort_invalid_uri = Lsp_test_helpers.lsp_doc_uri_of_path abort_invalid_path in
  let return_invalid_uri = Lsp_test_helpers.lsp_doc_uri_of_path return_invalid_path in
  ...
  Lsp_test_helpers.with_server
    ...
    expect_empty_diags_for_open ~srv ~uri:exit_uri ~timeout_s:open_timeout_s ~phase:"didOpen EXIT";
    ...
    expect_empty_diags_for_open ~srv ~uri:abort_uri ~timeout_s:open_timeout_s ~phase:"didOpen ABORT";

    ensure_budget "open stop";
    Lsp_test_helpers.send_notification srv ~method_:"textDocument/didOpen"
      ~params:(did_open_params ~uri:stop_uri ~version:1 ~text:stop_text);
    expect_empty_diags_for_open ~srv ~uri:stop_uri ~timeout_s:open_timeout_s ~phase:"didOpen STOP";

    ensure_budget "open exit invalid";
    Lsp_test_helpers.send_notification srv ~method_:"textDocument/didOpen"
      ~params:(did_open_params ~uri:exit_invalid_uri ~version:1 ~text:exit_invalid_text);
    wait_uri_diag_contains_or_fail
      ~srv
      ~uri:exit_invalid_uri
      ~timeout_s:authoritative_timeout_s
      ~phase:"didOpen EXIT invalid"
      ~needle:"EXIT is only valid inside a loop.";

    ensure_budget "open abort invalid";
    Lsp_test_helpers.send_notification srv ~method_:"textDocument/didOpen"
      ~params:(did_open_params ~uri:abort_invalid_uri ~version:1 ~text:abort_invalid_text);
    wait_uri_diag_contains_or_fail
      ~srv
      ~uri:abort_invalid_uri
      ~timeout_s:authoritative_timeout_s
      ~phase:"didOpen ABORT invalid"
      ~needle:"ABORT is only valid inside a procedure.";

    ensure_budget "open return invalid";
    Lsp_test_helpers.send_notification srv ~method_:"textDocument/didOpen"
      ~params:(did_open_params ~uri:return_invalid_uri ~version:1 ~text:return_invalid_text);
    wait_uri_diag_contains_or_fail
      ~srv
      ~uri:return_invalid_uri
      ~timeout_s:authoritative_timeout_s
      ~phase:"didOpen RETURN invalid"
    ...
    let first_broken =
      wait_uri_diags_or_fail ~srv ~uri:broken_uri ~timeout_s:open_timeout_s ~phase:"didOpen broken"
    in
    assert_no_false_control_keyword_diag ~phase:"didOpen broken" first_broken;
    if not (diags_contain_message ~needle:"Undefined procedure" first_broken) then
      wait_uri_diag_contains_or_fail
        ~srv
        ~uri:broken_uri
        ~timeout_s:authoritative_timeout_s
