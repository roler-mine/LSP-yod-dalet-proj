...
let () =
  ...
  let root =
    Lsp_test_helpers.mk_temp_dir "jovial-lsp-open-diag-compool-revalidate"
  in
    ...
    (fun srv ->
      ...
      ignore
        (Lsp_test_helpers.wait_for_publish_diagnostics_for_uri ~srv
           ~target_uri:pool_uri ~timeout_s:8.0);
      ...
      wait_for_metric_calls ~srv ~name:"diag.open.revalidate_reason_compool"
