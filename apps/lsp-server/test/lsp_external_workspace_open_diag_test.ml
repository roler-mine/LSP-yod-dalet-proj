...
let assert_no_false_control_keyword_diag ~(path : string)
  ...
  in
  if offending <> [] then
    failf "false control-keyword diagnostics in %s: %s" path
...
let () =
  ...
  | None ->
      print_endline
        "lsp_external_workspace_open_diag_test: skipped (set \
  ...
  | Some ws_path ->
      ...
      let hard_timeout_s =
        float_of_int
          (max 1
             (Lsp_test_helpers.getenv_int
                "JOVIAL_TEST_EXTERNAL_WS_HARD_TIMEOUT_S" ~default:900))
      in
      let ready_timeout_s =
        max 2.0
      ...
      let require_ready_convergence =
        getenv_bool "JOVIAL_TEST_EXTERNAL_WS_REQUIRE_READY_CONVERGENCE"
          ~default:false
      in
      ...
      let ensure_budget (phase : string) : unit =
        if Unix.gettimeofday () -. started > hard_timeout_s then
          failf
            "external workspace open-diagnostics test exceeded hard timeout \
             (%.1fs) at %s"
        ...
        (fun srv ->
          ...
          ignore
            (Lsp_test_helpers.initialize_and_open srv ~root_uri
               ~doc_uri:first_uri ~doc_text:first_text
               ~timeout_s:(timeout_for_path first));
          let first_diags =
            wait_uri_diags_or_fail ~srv ~uri:first_uri
              ~timeout_s:(timeout_for_path first) ~phase:first
          in
          ...
          |> List.iteri (fun idx path ->
              ...
              let diags =
                wait_uri_diags_or_fail ~srv ~uri
          ...
          if require_ready_convergence then (
            ...
            if not (startup_xmodule_diag_ready_of_report report) then
              failf
                "expected startup.components.xmoduleDiagReady=true after \
                 diagHoverReady");
      ...
      print_endline
        (Printf.sprintf "lsp_external_workspace_open_diag_test: ok (%d files)"
           (List.length files))
