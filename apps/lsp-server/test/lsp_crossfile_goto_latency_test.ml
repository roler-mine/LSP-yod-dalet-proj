...
let request_definition ~(srv : Lsp_test_helpers.server_proc) ~(id : int)
    ...
    Yojson.Safe.t * float =
  let params =
    Lsp_test_helpers.definition_request_params ~uri ~line ~character
  in
...
let () =
  ...
  let index_files =
    max 0
      (Lsp_test_helpers.getenv_int "JOVIAL_TEST_CROSSFILE_INDEX_FILES"
         ~default:1800)
  in
  let timeout_s =
    float_of_int
      (max 1
         (Lsp_test_helpers.getenv_int "JOVIAL_TEST_CROSSFILE_TIMEOUT_S"
            ~default:12))
  in
  let first_max_ms =
    float_of_int
      (max 1
         (Lsp_test_helpers.getenv_int "JOVIAL_TEST_CROSSFILE_FIRST_MAX_MS"
            ~default:2000))
  in
  let settle_timeout_s =
    float_of_int
      (max 1
         (Lsp_test_helpers.getenv_int "JOVIAL_TEST_CROSSFILE_SETTLE_TIMEOUT_S"
            ~default:4))
  in
  ...
  let ensure_budget (phase : string) : unit =
    ...
    if left <= 0.0 then
      failf "cross-file goto latency test exceeded hard timeout (%.1fs) at %s"
  ...
  let main_path, main_text, target_path =
    create_workspace ~root ~file_count:index_files
  in
  ...
  let line, col =
    Lsp_test_helpers.line_col_of_first main_text
      ~needle:"TARGET'VAL = TARGET'VAL + 1;"
  in
  ...
  Lsp_test_helpers.with_server ~server_path (fun srv ->
      ...
      let first_resp, first_ms =
        request_definition ~srv ~id:2 ~uri:doc_uri ~line ~character:def_char
          ~timeout_s:(min timeout_s (remaining_budget ()))
      in
      if first_ms > first_max_ms then
        failf "first cross-file definition too slow: %.1fms (limit %.1fms)"
      ...
      if not found_eventually then
        failf
          "cross-file definition did not converge to target within %.1fs \
           (target=%s)"
          settle_timeout_s target_uri;
