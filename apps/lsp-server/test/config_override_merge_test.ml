...
let () =
                    ...
                    ( "workspace",
                      `Assoc
                        [
                          ...
                          ( "manualRootFiles",
                            `List [ `String "main.j73"; `String "ops.j73" ] );
                        ] );
                    ( "background",
                      `Assoc
                        [
                          ("indexBudgetMs", `Int 21); ("diagBatchSize", `Int 9);
                        ] );
                    ( "server",
                      `Assoc
                        [
  ...
  let ws_settings =
    Lib.Workspace_settings.from_env () |> fun settings ->
    Lib.Workspace_settings.apply_client_overrides settings
      {
        Lib.Workspace_settings.workspace_diag_mode =
          overrides.workspace_diag_mode;
  ...
  let runtime_settings =
    Lib.Lsp_runtime_settings.from_env () |> fun settings ->
    Lib.Lsp_runtime_settings.apply_client_overrides settings overrides
  in
  expect_bool ~name:"workspace diagnostics override"
    (ws_settings.workspace_diag_mode = Lib.Workspace_settings.WorkspaceDiagsAll);
  expect_bool ~name:"workspace profile override"
    (ws_settings.workspace_profile_mode
   = Lib.Workspace_settings.ProfileModeLarge);
  expect_bool ~name:"root model override"
    (ws_settings.root_model = Lib.Workspace_settings.RootModelManual);
  expect_bool ~name:"manual root files override"
    (ws_settings.root_manual_files = [ "main.j73"; "ops.j73" ]);
  expect_bool ~name:"parse max file bytes override"
    (ws_settings.parse_file_max_bytes = 4096);
  expect_bool ~name:"pressure soft override" (ws_settings.pressure_soft_mb = 640);
  expect_bool ~name:"pressure critical override"
    (ws_settings.pressure_critical_mb = 896);
  expect_bool ~name:"background budget override"
    (runtime_settings.bg_tick_budget_ms = 21);
  expect_bool ~name:"background diagnostics batch override"
