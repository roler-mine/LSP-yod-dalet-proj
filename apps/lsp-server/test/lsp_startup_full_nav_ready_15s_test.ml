...
type ready_stage = StageDiagHoverReady | StageFullyNavigable
...
let wait_for_workspace_ready_stage ~(srv : Lsp_test_helpers.server_proc)
    ~(stage : ready_stage) ~(timeout_s : float) ~(on_tick : unit -> unit) :
    Yojson.Safe.t =
...
let parse_stage_readiness_payload ~(stage : ready_stage)
    (params : Yojson.Safe.t) : int * int * bool * bool =
  ...
  | `Assoc fields -> (
      ...
      | Some (`Assoc readiness) ->
          ...
          let stage_obj =
            ...
            | Some (`Assoc xs) -> xs
            | _ ->
                failf "workspaceReady payload missing readiness.stages.%s"
                  stage_name
          in
          let elapsed =
  ...
  | _ -> failf "workspaceReady payload is not a JSON object"

let result_field (resp : Yojson.Safe.t) : Yojson.Safe.t option =
  match resp with `Assoc fields -> List.assoc_opt "result" fields | _ -> None
...
let ensure_no_error ~(method_ : string) (resp : Yojson.Safe.t) : unit =
  ...
  | `Assoc fields -> (
      ...
      | None -> ()
      | Some err ->
          failf "request %s returned error: %s" method_
            (Yojson.Safe.to_string err))
  | _ -> failf "request %s response is not a JSON object" method_

let expect_non_null_result ~(method_ : string) (resp : Yojson.Safe.t) : unit =
  match result_field resp with
  | Some `Null -> failf "request %s returned null after startup-ready" method_
  | Some _ -> ()
  | None -> failf "request %s response missing result" method_

let completion_params ~(uri : string) ~(line : int) ~(character : int) :
    Yojson.Safe.t =
  `Assoc
    [
...
let prepare_rename_params ~(uri : string) ~(line : int) ~(character : int) :
    Yojson.Safe.t =
  `Assoc
    [
      ("textDocument", `Assoc [ ("uri", `String uri) ]);
...
let rename_params ~(uri : string) ~(line : int) ~(character : int)
...
let run_nav_request ~(srv : Lsp_test_helpers.server_proc) ~(id : int)
    ~(method_ : string) ~(params : Yojson.Safe.t) ~(timeout_s : float) : unit =
  let resp, elapsed_ms =
  ...
  if elapsed_ms > 2000.0 then
    failf "%s too slow post-ready: %.1fms (limit 2000ms)" method_ elapsed_ms
...
let () =
    ...
    float_of_int
      (max 1
         (Lsp_test_helpers.getenv_int
            "JOVIAL_TEST_STARTUP_FULL_NAV_HARD_TIMEOUT_S" ~default:180))
  in
  let workspace_files =
    max 1
      (Lsp_test_helpers.getenv_int "JOVIAL_TEST_STARTUP_FULL_NAV_FILES"
         ~default:120)
  in
  ...
  let ensure_budget (phase : string) =
    if remaining_budget () <= 0.0 then
      failf "startup full-nav ready test exceeded hard timeout (%.1fs) at %s"
        hard_timeout_s phase
  in

  let root = Lsp_test_helpers.mk_temp_dir "jovial-lsp-startup-full-nav-ready" in
  let main_path, main_text =
    create_workspace ~root ~file_count:workspace_files
  in
  ...
  Lsp_test_helpers.with_server
    ...
    (fun srv ->
      ...
      let drive_startup_traffic () =
        ...
        (try
           ignore
             (Lsp_test_helpers.request_timed srv ~id:id1
                ~method_:"textDocument/hover"
                ~params:
                  (hover_params ~uri:doc_uri ~line:lib_line
                     ~character:(lib_col + 1))
                ~timeout_s:1.0)
         with _ -> ());
        let id2 = !req_id in
        ...
        try
          ignore
            (Lsp_test_helpers.request_timed srv ~id:id2
               ~method_:"textDocument/definition"
               ~params:
                 (Lsp_test_helpers.definition_request_params ~uri:doc_uri
                    ~line:lib_line ~character:(lib_col + 1))
               ~timeout_s:1.0)
        with _ -> ()
      in
      ensure_budget "wait diagHoverReady";
      let diag_params =
        wait_for_workspace_ready_stage ~srv ~stage:StageDiagHoverReady
      ...
      let full_params =
        wait_for_workspace_ready_stage ~srv ~stage:StageFullyNavigable
      ...
      run_nav_request ~srv ~id:!req_id ~method_:"textDocument/definition"
        ~params:
          (Lsp_test_helpers.definition_request_params ~uri:doc_uri
             ~line:lib_line ~character:(lib_col + 1))
        ~timeout_s:request_timeout_s;
      incr req_id;
      run_nav_request ~srv ~id:!req_id ~method_:"textDocument/implementation"
        ~params:
          (Lsp_test_helpers.definition_request_params ~uri:doc_uri
             ~line:lib_line ~character:(lib_col + 1))
        ~timeout_s:request_timeout_s;
      incr req_id;
      run_nav_request ~srv ~id:!req_id ~method_:"textDocument/references"
        ~params:
          (references_params ~uri:doc_uri ~line:lib_line ~character:(lib_col + 1))
        ~timeout_s:request_timeout_s;
      incr req_id;
      run_nav_request ~srv ~id:!req_id ~method_:"textDocument/hover"
        ~params:
          (hover_params ~uri:doc_uri ~line:lib_line ~character:(lib_col + 1))
        ~timeout_s:request_timeout_s;
      incr req_id;
      run_nav_request ~srv ~id:!req_id ~method_:"textDocument/completion"
        ~params:
          (completion_params ~uri:doc_uri ~line:completion_line
             ~character:(completion_col + 3))
        ~timeout_s:request_timeout_s;
      incr req_id;
      run_nav_request ~srv ~id:!req_id ~method_:"textDocument/prepareRename"
        ~params:
          (prepare_rename_params ~uri:doc_uri ~line:target_line
             ~character:(target_col + 1))
        ~timeout_s:request_timeout_s;
      incr req_id;
      run_nav_request ~srv ~id:!req_id ~method_:"textDocument/rename"
        ~params:
          (rename_params ~uri:doc_uri ~line:target_line
             ~character:(target_col + 1) ~new_name:"TARGET_RENAMED")
        ~timeout_s:request_timeout_s;

      ensure_budget "shutdown";
      Lsp_test_helpers.shutdown_and_exit srv
        ~timeout_s:(min 3.0 (remaining_budget ()));
