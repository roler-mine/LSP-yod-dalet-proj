...
let find_update_for_uri
    (updates:(T.DocumentUri.t * T.Diagnostic.t list) list)
    ~(uri:T.DocumentUri.t)
...
let () =
  ...
  let ensure_budget (phase:string) : unit =
    ...
    if left <= 0.0 then
      failf
        "workspace diagnostics latest-wins test exceeded hard timeout (%.1fs) at %s"
        hard_timeout_s
        phase
  in
  ...
  let baseline_diags =
    ...
    | Some ds -> ds
    | None -> failf "baseline diagnostics update for broken file was not published"
  in
  if not (has_error_diag baseline_diags) then
    failf "expected baseline broken file diagnostics to include at least one error";
  ...
  if final_diags <> [] then
    failf
      "expected latest-wins diagnostics payload to be empty after fix, got %d diagnostics"
