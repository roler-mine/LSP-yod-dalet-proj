...
let watched_files_params (uris:string list) : Yojson.Safe.t =
  `Assoc [ "changes", `List (List.map (fun u -> watched_change_obj ~uri:u) uris) ]
...
let () =
  ...
  in
  let timeout_s =
    float_of_int (max 2 (getenv_int "JOVIAL_TEST_LARGE_WS_SURV_TIMEOUT_S" ~default:18))
  in
  let file_count =
  ...
  let ensure_budget (phase:string) : unit =
    if remaining_budget () <= 0.0 then
      failf "large workspace survival test exceeded hard timeout (%.1fs) at %s" hard_timeout_s phase
  in
  ...
  let changed_uris =
    extra_paths
    |> List.map Lsp_test_helpers.lsp_doc_uri_of_path
  in
