...
let command_result_assoc
  ...
  | `Assoc fields -> (
      ...
      | Some other ->
          failf
            "command %s returned non-object result: %s"
            command
            (Yojson.Safe.to_string other)
      | None ->
          failf "command %s response missing result" command)
  | _ ->
      failf "command %s response was not an object" command
...
let compools_state_of_report (report_fields:(string * Yojson.Safe.t) list) : compools_state =
  let compools =
...
let request_debug_report
    ~(srv:Lsp_test_helpers.server_proc)
    ~(id:int)
    ~(uri:string)
    ~(timeout_s:float)
  : compools_state =
  command_result_assoc
    ~srv
    ~id
    ~command:"jovial.debugReport"
...
let wait_for_converged_definition
    ~(srv:Lsp_test_helpers.server_proc)
    ~(doc_uri:string)
    ~(line:int)
    ~(character:int)
  ...
  let rec loop req_id =
    if Unix.gettimeofday () >= deadline then
      failf
        "definition did not converge to %s within %.1fs"
        target_uri
        timeout_s;
    let resp, _ =
      request_definition
...
let replace_with_new_workspace ~(root:string) ~(old_pool:string) : string * string * string =
...
let () =
    ...
    float_of_int
      (max 1
         (Lsp_test_helpers.getenv_int
            "JOVIAL_TEST_STALE_RECONCILE_HARD_TIMEOUT_S"
            ~default:180))
  in
  let converge_timeout_s =
    max 1.0
      (match Sys.getenv_opt "JOVIAL_TEST_STALE_RECONCILE_CONVERGE_TIMEOUT_S" with
       | None -> 12.0
    ...
    (fun srv ->
      ...
      ignore
        (Lsp_test_helpers.initialize_and_open
           srv
           ~root_uri
           ~doc_uri:old_main_uri
           ~doc_text:old_main_text
           ~timeout_s:4.0);
      wait_until_index_complete
        ~srv
        ~doc_uri:old_main_uri
  ...
  let new_main, new_main_text, new_pool = replace_with_new_workspace ~root ~old_pool in
  ...
  let line, col =
    Lsp_test_helpers.line_col_of_first new_main_text ~needle:"NEW'VAL = NEW'VAL + 1;"
  in
    ...
    (fun srv ->
      ...
      ignore
        (Lsp_test_helpers.initialize_and_open
           srv
           ~root_uri
           ~doc_uri:new_main_uri
           ~doc_text:new_main_text
           ~timeout_s:4.0);
      ...
      if not (response_has_result_uri first_resp ~uri:new_pool_uri) then
        wait_for_converged_definition
          ~srv
          ~doc_uri:new_main_uri
          ~line
          ~character:(col + 2)
          ~target_uri:new_pool_uri
      ...
      if final_state.stale_pruned_count <= 0 then
        failf "expected stalePrunedCount > 0 after stale checkpoint reconciliation";
      if final_state.count < 1 || final_state.sources < 2 then
        failf
          "unexpected compools/sources after reconciliation (count=%d sources=%d)"
          final_state.count
          final_state.sources;
      if not (List.exists (fun n -> String.uppercase_ascii n = "NEWPOOL") final_state.sample_names) then
        failf "reconciled compool sample did not include NEWPOOL";
      if List.exists (fun n -> String.uppercase_ascii n = "OLDPOOL") final_state.sample_names then
        failf "reconciled compool sample still contains stale OLDPOOL entry";
