...
let () =
  ...
  in
  let index_files =
    max 0 (Lsp_test_helpers.getenv_int "JOVIAL_TEST_STARTUP_INDEX_FILES" ~default:2200)
  in
  let timeout_s =
    float_of_int (max 1 (Lsp_test_helpers.getenv_int "JOVIAL_TEST_STARTUP_TIMEOUT_S" ~default:12))
  in
  let max_startup_nav_ms =
    float_of_int (max 1 (Lsp_test_helpers.getenv_int "JOVIAL_TEST_STARTUP_NAV_MAX_MS" ~default:2000))
  in
  ...
  let ensure_budget (phase:string) : unit =
    ...
    if left <= 0.0 then
      failf "startup open-file nav test exceeded hard timeout (%.1fs) at %s" hard_timeout_s phase
  in
  ...
  let line, col = Lsp_test_helpers.line_col_of_first main_text ~needle:"TARGET = TARGET + 1;" in
  Lsp_test_helpers.with_server ~server_path (fun srv ->
    ...
    if def_ms > max_startup_nav_ms then
      failf
        "startup definition too slow: %.1fms (limit %.1fms)"
        def_ms
        max_startup_nav_ms;
    if hover_ms > max_startup_nav_ms then
      failf
        "startup hover too slow: %.1fms (limit %.1fms)"
        hover_ms
        max_startup_nav_ms;
