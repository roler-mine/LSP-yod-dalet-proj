...
let symbols_and_ids (result_fields : (string * Yojson.Safe.t) list) :
    Yojson.Safe.t list * (string, bool) Hashtbl.t =
  let symbols =
  ...
  List.iter
    (function
      | `Assoc sf -> (
          ...
          | _ ->
              failf "lsif symbol missing non-empty id: %s"
                (Yojson.Safe.to_string (`Assoc sf)))
      | other ->
          failf "lsif symbol entry is not an object: %s"
...
let assert_key_index_shape ~(ids : (string, bool) Hashtbl.t)
    (result_fields : (string * Yojson.Safe.t) list) : unit =
  let key_index =
    ...
    (function
      | `Assoc kf ->
          let key =
            ...
            | _ ->
                failf "lsif keyIndex entry missing key: %s"
                  (Yojson.Safe.to_string (`Assoc kf))
          in
          ignore key;
          let sym_ids =
            ...
            | Some (`List xs) -> xs
            | _ ->
                failf "lsif keyIndex entry missing symbolIds[]: %s"
                  (Yojson.Safe.to_string (`Assoc kf))
          in
          if sym_ids = [] then
            failf "lsif keyIndex entry had empty symbolIds[]: %s"
              (Yojson.Safe.to_string (`Assoc kf));
          List.iter
            (function
              | `String id when Hashtbl.mem ids id -> ()
              | `String id ->
                  failf "keyIndex referenced unknown symbol id %s" id
              | other ->
                  failf "keyIndex symbolIds contained non-string: %s"
                    (Yojson.Safe.to_string other))
            sym_ids
      | other ->
          failf "lsif keyIndex entry is not an object: %s"
            (Yojson.Safe.to_string other))
    key_index
...
let () =
  ...
  let timeout_s =
    float_of_int
      (max 1
         (Lsp_test_helpers.getenv_int "JOVIAL_TEST_LSIF_PAYLOAD_TIMEOUT_S"
            ~default:10))
  in
  ...
  let ensure_budget (phase : string) : unit =
    if remaining_budget () <= 0.0 then
      failf "lsif payload shape test exceeded hard timeout (%.1fs) at %s"
  ...
  Lsp_test_helpers.with_server ~server_path (fun srv ->
      ...
      let rec poll_index next_id deadline =
        if Unix.gettimeofday () >= deadline then
          failf
            "lsif payload shape test timed out waiting for non-empty symbols[]";
        let resp, _ =
