...
let method_params ~(uri:string) ~(line:int) ~(character:int) : Yojson.Safe.t =
  `Assoc [
...
let () =
  ...
  in
  let timeout_s =
    float_of_int (max 1 (Lsp_test_helpers.getenv_int "JOVIAL_TEST_DECL_TYPEDEF_TIMEOUT_S" ~default:10))
  in
  ...
  let ensure_budget (phase:string) : unit =
    if remaining_budget () <= 0.0 then
      failf "declaration/typeDefinition test exceeded hard timeout (%.1fs) at %s" hard_timeout_s phase
  in
  ...
  let decl_line, decl_col = Lsp_test_helpers.line_col_of_first main_text ~needle:"HELPER();" in
  let ty_line, ty_col = Lsp_test_helpers.line_col_of_first main_text ~needle:"ITEM V MYTYPE;" in
  ...
  Lsp_test_helpers.with_server ~server_path (fun srv ->
    ...
    let decl_resp, _ =
      Lsp_test_helpers.request_timed srv
        ...
        ~params:(method_params ~uri:doc_uri ~line:decl_line ~character:decl_col)
        ~timeout_s:(min timeout_s (remaining_budget ()))
    in
    ...
    if not (response_has_result_uri decl_resp ~uri:doc_uri) then
      failf "declaration did not resolve to current document: %s" (Yojson.Safe.to_string decl_resp);

    let td_resp, _ =
      Lsp_test_helpers.request_timed srv
        ~id:3
        ~method_:"textDocument/typeDefinition"
        ~params:(method_params ~uri:doc_uri ~line:ty_line ~character:type_char)
        ~timeout_s:(min timeout_s (remaining_budget ()))
    in
    if not (no_error td_resp) then
      failf "typeDefinition returned error: %s" (Yojson.Safe.to_string td_resp);
    if not (response_has_result_uri td_resp ~uri:doc_uri) then
      failf "typeDefinition did not resolve to current document: %s" (Yojson.Safe.to_string td_resp);
