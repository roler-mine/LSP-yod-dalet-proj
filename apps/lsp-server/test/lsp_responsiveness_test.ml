...
let () =
  ...
  let index_files =
    max 0
      (Lsp_test_helpers.getenv_int "JOVIAL_TEST_RESP_INDEX_FILES" ~default:2000)
  in
  let timeout_s =
    float_of_int
      (max 1
         (Lsp_test_helpers.getenv_int "JOVIAL_TEST_RESP_TIMEOUT_S" ~default:12))
  in
  let max_init_ms =
    float_of_int
      (max 1
         (Lsp_test_helpers.getenv_int "JOVIAL_TEST_RESP_MAX_INIT_MS"
            ~default:2500))
  in
  let max_definition_ms =
    float_of_int
      (max 1
         (Lsp_test_helpers.getenv_int "JOVIAL_TEST_RESP_MAX_DEFINITION_MS"
            ~default:1000))
  in
  ...
  let ensure_budget (phase : string) : unit =
    ...
    if left <= 0.0 then
      failf "indexing responsiveness test exceeded hard timeout (%.1fs) at %s"
  ...
  Lsp_test_helpers.with_server ~server_path (fun srv ->
      ...
      let init_ms =
        Lsp_test_helpers.initialize_and_open srv ~root_uri ~doc_uri
          ~doc_text:main_text ~timeout_s:init_timeout_s
      in
      ...
      if init_ms > max_init_ms then
        failf "initialize too slow: %.1fms (limit %.1fms, workspace files=%d)"
          init_ms max_init_ms index_files;
      if def_ms > max_definition_ms then
        failf "definition too slow: %.1fms (limit %.1fms)" def_ms
          max_definition_ms;
