...
let index_startup_disable = Env_utils.flag "JOVIAL_INDEX_STARTUP_DISABLE" ~default:false
let index_startup_dirs = Env_utils.nonneg_int "JOVIAL_INDEX_STARTUP_DIRS" ~default:0
let index_startup_files = Env_utils.nonneg_int "JOVIAL_INDEX_STARTUP_FILES" ~default:0
...
let index_stale_reconcile_min_interval_ms =
  max 100
    (Env_utils.nonneg_int "JOVIAL_INDEX_STALE_RECONCILE_MIN_INTERVAL_MS" ~default:900)
let index_reconcile_escalate_dirs =
  max 1 (Env_utils.nonneg_int "JOVIAL_INDEX_RECONCILE_ESCALATE_DIRS" ~default:24)
let index_reconcile_escalate_files =
  max 1 (Env_utils.nonneg_int "JOVIAL_INDEX_RECONCILE_ESCALATE_FILES" ~default:3000)
...
let didchange_semi_force_full_every =
  max 1 (Env_utils.nonneg_int "JOVIAL_DIDCHANGE_SEMI_FORCE_FULL_EVERY" ~default:20)
...
let didchange_defer_parse_force_full_every =
  max 1 (Env_utils.nonneg_int "JOVIAL_DIDCHANGE_DEFER_FORCE_FULL_EVERY" ~default:24)
...
let nav_quick_scan_total_bytes =
  Env_utils.nonneg_int "JOVIAL_NAV_QUICK_SCAN_TOTAL_BYTES" ~default:(12 * 1024 * 1024)
...
let nav_miss_import_scan_max_chars =
  max 4096
    (Env_utils.nonneg_int "JOVIAL_NAV_MISS_IMPORT_SCAN_MAX_CHARS" ~default:262144)
