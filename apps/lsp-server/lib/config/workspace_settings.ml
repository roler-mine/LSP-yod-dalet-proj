...
let workspace_profile_mode_of_string (raw:string) : workspace_profile_mode option =
  match String.lowercase_ascii (String.trim raw) with
...
let from_env () : t =
  let startup_target_ms =
    max 1000
      (Env_utils.nonneg_int "JOVIAL_STARTUP_TARGET_MS" ~default:startup_target_ms_default)
  in
  let startup_diag_hover_target_ms =
    max 1000
      (Env_utils.nonneg_int
  ...
  let sched_open_doc_min_share_pct =
    let n = Env_utils.nonneg_int "JOVIAL_SCHED_OPEN_DOC_MIN_SHARE_PCT" ~default:50 in
    min 100 (max 0 n)
  in
  {
    ...
    parse_file_max_bytes =
      max 1 (Env_utils.nonneg_int "JOVIAL_PARSE_FILE_MAX_BYTES" ~default:16777216);
    bg_large_file_bytes =
    ...
    pressure_soft_mb =
      max 64
        (Env_utils.nonneg_int "JOVIAL_PRESSURE_SOFT_MB" ~default:pressure_soft_mb_default);
    pressure_critical_mb =
      max 64
        (Env_utils.nonneg_int
           "JOVIAL_PRESSURE_CRITICAL_MB"
    ...
    graph_requeue_cooldown_ms =
      max 0 (Env_utils.nonneg_int "JOVIAL_GRAPH_REQUEUE_COOLDOWN_MS" ~default:400);
    root_closure_max_depth =
      max 1 (Env_utils.nonneg_int "JOVIAL_ROOT_CLOSURE_MAX_DEPTH" ~default:4);
    root_closure_target_files =
      max 8 (Env_utils.nonneg_int "JOVIAL_ROOT_CLOSURE_TARGET_FILES" ~default:256);
    skeleton_prefix_bytes =
      max 1024 (Env_utils.nonneg_int "JOVIAL_SKELETON_PREFIX_BYTES" ~default:262144);
