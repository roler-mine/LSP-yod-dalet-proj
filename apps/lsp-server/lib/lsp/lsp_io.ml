...
type header_result = HeaderEof | HeaderLength of int | HeaderInvalid of string
...
let read_headers ic : header_result =
  let rec loop ~(seen_any : bool) (len : int option) =
    ...
    | exception End_of_file ->
        if seen_any then
          HeaderInvalid "unexpected EOF while reading LSP headers"
        else HeaderEof
    | line ->
        ...
        if line = "" then
          ...
          | Some n ->
              HeaderInvalid (Printf.sprintf "invalid Content-Length: %d" n)
          | None -> HeaderInvalid "missing Content-Length header"
        else
          let len =
            match parsed_len with Some n when n >= 0 -> Some n | _ -> len
          in
...
let discard_bytes (ic : in_channel) (len : int) : bool =
  ...
  while !remaining > 0 && !ok do
    ...
    if got <= 0 then ok := false else remaining := !remaining - got
  done;
  !ok
...
let read_message_with_limit (ic : in_channel) ~(max_len : int) : read_result =
  ...
  | HeaderLength len -> (
      if len > max_len then
        ...
        with End_of_file -> `Invalid "unexpected EOF while reading LSP body")
