             ...
             if not version_ok || not root_ok then
               ...
               (match field "sourcesByDir" with
                | Some (`List dirs) ->
                    List.iter (function
                      | `Assoc fields ->
                          (match List.assoc_opt "dir" fields, List.assoc_opt "paths" fields with
...
let remove_source_dir_entry (t:t) ~(path_key:string) ~(path:string) : unit =
  ...
  if dir_key <> "" then
    ...
    | Some set ->
        ...
        if Hashtbl.length set = 0 then
          Hashtbl.remove t.sources_by_dir dir_key
...
let remove_directory_tree (t:t) (dir:string) : bool =
  ...
  let prefix_dir =
    ...
    if n > 0 && prefix.[n - 1] = '/' then String.sub prefix 0 (n - 1) else prefix
  in
  let files = ref [] in
  Hashtbl.iter (fun dir_key set ->
