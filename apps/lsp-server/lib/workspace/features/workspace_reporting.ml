...
let semantic_tokens_for_doc (ws:t) (doc:Document.t) ~(range:T.Range.t option) : semantic_token list =
      ...
      | Some snap when snap.Doc_snapshot.doc_rev = doc.Document.rev ->
          (match snap.Doc_snapshot.semantic_tokens_full with
           ...
           | Some toks ->
               ...
               Some (List.map semantic_token_of_snapshot toks |> List.filter in_range))
      | _ -> None
    else
      None
  in
  ...
  | None ->
      ...
      if ws.sem_store_enabled && range = None then
        (match Semantic_store.snapshot_for_uri ws.semantic_store ~uri:doc.Document.uri with
         | Some snap when snap.Doc_snapshot.doc_rev = doc.Document.rev ->
             Doc_snapshot.set_semantic_tokens_full snap (List.map snapshot_token_of_semantic toks)
         | _ -> ());
      toks
