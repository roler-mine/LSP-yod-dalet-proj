...
let text_region_equals (text:string) ~(offset:int) ~(replacement:string) : bool =
...
let apply_one_change (text : string) (idx : Text_index.t) (c : T.TextDocumentContentChangeEvent.t)
  ...
  | Some r ->
      ...
      | Some a, Some b ->
          ...
          if replaced_len = String.length c.text
...
let apply_changes_no_reparse ~(changes:T.TextDocumentContentChangeEvent.t list) (doc : t) : t =
  if changes = [] then doc
  else apply_changes doc changes
