...
let pct_encode_path (s : string) : string =
  ...
  String.iter
    (fun c ->
      if is_unreserved c || c = '/' || c = ':' then Buffer.add_char b c
      else (
        Buffer.add_char b '%';
...
let docuri_to_string (u : T.DocumentUri.t) : string =
  try match T.DocumentUri.yojson_of_t u with `String s -> s | _ -> ""
  with _ -> ""
  ...
  | Some rest ->
      ...
      if Sys.win32 then
        if dec_authority <> "" && dec_authority <> "localhost" then
          ...
          let p =
            if
              String.length dec_path >= 3
              && dec_path.[0] = '/'
              && is_drive_path
                   (String.sub dec_path 1 (String.length dec_path - 1))
            then String.sub dec_path 1 (String.length dec_path - 1)
            else dec_path
          in
          Some (String.map (fun c -> if c = '/' then '\\' else c) p)
      else if dec_authority <> "" && dec_authority <> "localhost" then
