...
let set_add (set:str_set) (k:string) : unit =
  Hashtbl.replace set k true

let set_remove (set:str_set) (k:string) : unit =
  Hashtbl.remove set k

let set_is_empty (set:str_set) : bool =
  Hashtbl.length set = 0
...
let rebuild_key_set (s:t) (key:string) : unit =
  ...
  if key = "" then ()
    ...
    Hashtbl.iter (fun sym_id defs ->
      if List.exists (fun (d:Doc_snapshot.nav_def) -> normalize_key d.key = key) defs then
        Hashtbl.replace set sym_id true
    ) s.defs_by_sym_id;
    if set_is_empty set then
      Hashtbl.remove s.sym_ids_by_key key
    else
      Hashtbl.replace s.sym_ids_by_key key set
...
let remove_uri_key (s:t) (uk:string) : unit =
  ...
  | Some old ->
      ...
      (match old.path_key with
       ...
       | Some path_key ->
           (match Hashtbl.find_opt s.doc_set_by_path_key path_key with
            ...
            | Some set ->
                set_remove set uk;
                if set_is_empty set then Hashtbl.remove s.doc_set_by_path_key path_key));
        ...
        | Preprocess.Compool ->
            ...
            if ck <> "" then
              (match Hashtbl.find_opt s.reverse_imports_by_compool ck with
               ...
               | Some set ->
                   set_remove set uk;
                   if set_is_empty set then Hashtbl.remove s.reverse_imports_by_compool ck)
      ) old.imports;

      List.iter (fun (sym_id, _) ->
        match Hashtbl.find_opt s.defs_by_sym_id sym_id with
        | None -> ()
        | Some defs ->
            let defs' = List.filter (fun (d:Doc_snapshot.nav_def) -> uri_key d.uri <> uk) defs in
      ...
      List.iter (fun (sym_id, _) ->
        ...
        | None -> ()
        | Some occs ->
            let occs' = List.filter (fun ((u, _):Doc_snapshot.nav_occ) -> uri_key u <> uk) occs in
...
let resolve_symbol_at (s:t) ~(uri:T.DocumentUri.t) ~(pos:T.Position.t) : string option =
  match snapshot_for_uri s ~uri with
  | None -> None
  | Some snap ->
      let best : (string * int) option ref = ref None in
      List.iter (fun (sym_id, occs) ->
        List.iter (fun ((u, loc):Doc_snapshot.nav_occ) ->
...
let uris_importing_compool (s:t) ~(compool_key:string) : T.DocumentUri.t list =
...
let invalidate_path_and_dependents (s:t) ~(path_key:string) : T.DocumentUri.t list =
  let direct_exact =
    match Hashtbl.find_opt s.doc_set_by_path_key path_key with
    | None -> []
  ...
  let removed = ref [] in
  Hashtbl.iter (fun uk _ ->
    match Hashtbl.find_opt s.snapshot_by_uri uk with
    | None -> ()
    | Some snap ->
        removed := snap.uri :: !removed
  ) to_remove;
