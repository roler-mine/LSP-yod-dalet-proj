...
let metric_for (name:string) : metric_mut =
  ...
  | Some m -> m
  | None ->
      let m : metric_mut = { calls = 0; total_ms = 0.0; max_ms = 0.0; last_ms = 0.0 } in
...
let snapshot_json () : Yojson.Safe.t =
  let entries =
    Hashtbl.fold (fun name m acc ->
      ...
      let avg_ms =
        if snap.calls <= 0 then 0.0 else snap.total_ms /. float_of_int snap.calls
      in
      (`Assoc [
         "name", `String name;
