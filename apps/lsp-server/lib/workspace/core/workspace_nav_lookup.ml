...
let def_to_loc_json (d : def) : Yojson.Safe.t = location_json ~uri:d.uri d.loc
...
let rec expr_to_compact_string (e : Ast.expr Ast.node) : string =
  ...
  | Ast.EField { base; field } -> expr_to_compact_string base ^ "." ^ field.v
  | Ast.EAt { field; ptr } ->
      expr_to_compact_string field ^ " @ " ^ expr_to_compact_string ptr
  | Ast.EDeref { ptr } -> "@ " ^ expr_to_compact_string ptr
...
let find_proc_def_name_offsets ~(upper_text : string) ~(key : string) :
    (int * int) option =
  if key = "" then None
  else
    ...
    let rec loop from =
      ...
      | Some i ->
          ...
          let after_ok =
            name_e >= String.length upper_text
            || not (is_ident_char upper_text.[name_e])
          in
          if before_ok && after_ok then Some (name_s, name_e) else loop (i + 1)
    in
    loop 0

let find_proc_decl_name_offsets ~(upper_text : string) ~(key : string) :
    (int * int) option =
  if key = "" then None
  else
    let pat = "PROC " ^ key in
  ...
  then []
    ...
    if indexed_hits <> [] then indexed_hits
      ...
      | Some idx ->
          ...
          let mk_quick_hit ~(path : string) ~(text : string) ~(s : int)
              ~(e : int) : def =
          ...
          let rec scan scanned scanned_bytes (fallback : def option) = function
            | [] -> ( match fallback with None -> [] | Some d -> [ d ])
            ...
            | path :: tl ->
                ...
                if is_current then scan scanned scanned_bytes fallback tl
                  ...
                  if Hashtbl.mem ws.files path_key then
                    scan scanned scanned_bytes fallback tl
                  else
                    let offset =
                      match
                    ...
                    with
                    | None -> scan (scanned + 1) scanned_bytes fallback tl
                    | Some (text, next_offset) -> (
                        ...
                        if text_len = 0 then
                          ...
                          if next_bytes > scan_total_budget then
                            ...
                            match
                              find_proc_def_name_offsets ~upper_text ~key
                            with
                            | Some (s, e) -> [ mk_quick_hit ~path ~text ~s ~e ]
                            | None ->
                                let fallback =
                                  match fallback with
                                  | Some _ -> fallback
                                  | None -> (
                                      match
                                        find_proc_decl_name_offsets ~upper_text
                                          ~key
                                      with
                                      | None -> None
                                      | Some (s, e) ->
                                          Some (mk_quick_hit ~path ~text ~s ~e))
                                in
...
let perf_stats_json (_ws : t) : Yojson.Safe.t = Perf_stats.snapshot_json ()
