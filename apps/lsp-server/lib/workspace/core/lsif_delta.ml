type t = { mutable revision : int; symbols : (string, Yojson.Safe.t) Hashtbl.t }
...
let symbols_of_index_json (j : Yojson.Safe.t) :
  ...
  let symbols =
    match List.assoc_opt "symbols" fields with Some (`List xs) -> xs | _ -> []
  in
  List.iter
    (fun item ->
      match item with
...
let to_json_string (j : Yojson.Safe.t) : string = Yojson.Safe.to_string j
