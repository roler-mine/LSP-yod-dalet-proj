(* Module overview: Background scheduler for parsing, graph work, diagnostics, and readiness updates. *)
...
let define_signature (defines : Preprocess.define list) : string list =
  ...
  |> List.map (fun (d : Preprocess.define) ->
      Printf.sprintf "define:%s:%b:%s:%s" (normalize_name d.key) d.requires_call
  ...
  | Some { Document.parsed_syntax = Some syntax; _ } ->
      let declaration_units =
        ...
        |> List.filter_map (fun (unit_ : Syntax_cache.syntax_unit) ->
            ...
            | Syntax_cache.Declaration -> Some ("decl:" ^ unit_.digest)
            | _ -> None)
      in
      let skeleton_symbols =
        ...
        |> List.map (fun (symbol : Syntax_cache.skeleton_symbol) ->
            Printf.sprintf "sym:%s:%d:%s:%b:%b:%s"
              ...
              (Option.value symbol.sk_container ~default:"" |> normalize_name)
...
let normalized_file_changed (old_file : string option)
    (new_file : string option) : bool =
  match (old_file, new_file) with
  | None, None -> false
  | Some old_path, Some new_path ->
      normalize_path_key old_path <> normalize_path_key new_path
  | _ -> true
...
let doc_store_diff ~(old_doc : Document.t option) ~(new_doc : Document.t) :
  ...
  | Some old_doc ->
      ...
      in
      let file_changed =
        normalized_file_changed old_doc.Document.file new_doc.Document.file
      in
      let same_current_text =
        (old_doc.Document.text == new_doc.Document.text
        || old_doc.Document.text = new_doc.Document.text)
        && old_doc.Document.parse_rev = old_doc.Document.rev
        && new_doc.Document.parse_rev = new_doc.Document.rev
        && not file_changed
      in
      let declaration_signature_changed =
        if same_current_text then false
        else declaration_signature old_doc <> declaration_signature new_doc
      in
      ...
      let has_compool = has_compool_doc old_doc || has_compool_doc new_doc in
      {
        structural_changed =
...
let update_module_summary_cache_for_doc (ws : t) ~(old_doc : Document.t option)
    (doc : Document.t) (summary : Module_summary.t) : unit =
  (match old_doc with
  | Some old_doc -> (
...
let resolve_direct_compool_path (ws : t) (name : string) : string option =
...
let direct_import_paths (ws : t) (doc : Document.t) : string list =
  ...
  List.iter
    (fun (imp : Preprocess.import) ->
      match imp.kind with
      | Preprocess.Compool -> (
          match resolve_direct_compool_path ws imp.name with
          | None -> ()
          | Some p -> Hashtbl.replace acc (normalize_path_key p) p))
    (Document.imports doc);
  Hashtbl.fold (fun _ p xs -> p :: xs) acc []

let direct_import_ready (ws : t) (name : string) : bool =
  let key = normalize_name name in
  if key = "" then true
  else
    match find_open_compool_doc_by_key ws key with
    | Some doc -> Document.current_parse doc <> None
    | None -> (
        match resolve_direct_compool_path ws key with
        | None -> false
        | Some path -> (
...
let direct_imports_ready_for_doc (ws : t) (doc : Document.t) : bool =
  Document.imports doc
  |> List.for_all (fun (imp : Preprocess.import) ->
      match imp.kind with
      | Preprocess.Compool -> direct_import_ready ws imp.name)

let semantic_snapshot_current_for_doc (ws : t) (doc : Document.t) : bool =
  match
    Semantic_store.snapshot_for_uri ws.semantic_store ~uri:doc.Document.uri
  with
  | Some snap ->
      snap.Semantic_store.Snapshot.doc_rev = doc.Document.parse_rev
    ...
    | _
      when String.length doc.Document.text > ws.full_semantic_tokens_max_bytes
        ...
        Workspace_nav_model.upsert_semantic_snapshot_for_doc_with_skeleton ws
          doc
    | _
      when Document.has_current_parse doc && direct_imports_ready_for_doc ws doc
      -> (
        try
          Perf_stats.time "nav.index_doc" (fun () ->
              let nav =
                Perf_stats.time "nav.index_doc.build" (fun () ->
                    Workspace_nav_model.build_doc_nav ws doc)
              in
              Workspace_nav_model.upsert_semantic_snapshot_for_doc_with_nav ws
                doc nav)
        with _ ->
          Perf_stats.tick "query.cache.index_failed";
          Perf_log.log_event "semantic_nav_snapshot_failed"
...
let store_doc_fast ?(preserve_current_semantic_snapshot : bool = false) (ws : t)
  ...
  let old_doc =
    ...
    | None -> (
        match doc.Document.file with
        | None -> None
        | Some path -> Hashtbl.find_opt ws.files (normalize_path_key path))
  in
  ignore
    (install_doc_surface ~touch_bg_parsed:false
       ~enqueue_importer_revalidate:false ~preserve_current_semantic_snapshot ws
       ~uri ~old_doc doc);
  match doc.Document.file with
  | None -> ()
  | Some f ->
...
let background_doc_with_diags
    ?(semantic_mode : semantic_validation_mode = SemanticFull) (ws : t)
    (doc : Document.t) : Document.t =
  validated_doc_with_diags ~import_lookup_pump:false ~semantic_mode ws doc

let background_doc_for_indexing (ws : t) (doc : Document.t) : Document.t =
  if ws.startup_fully_nav_ready_ms = None then
    background_doc_with_diags ~semantic_mode:(SemanticRangeSemi zero_lsp_range)
      ws doc
  else background_doc_with_diags ws doc
...
let refresh_open_doc_diags ?(import_lookup_pump : bool = false) (ws : t)
  ...
  | Some doc ->
      ...
      refresh_semantic_nav_snapshot_for_doc ws refreshed;
      maybe_shed_open_doc_parse_state ws ~prefer_uri:(Some uri);
...
let parse_doc_worker ~(max_bytes : int) ~(uri : T.DocumentUri.t)
  ...
  if is_parse_guard_exceeded ~max_bytes ~text_len:(String.length text) then
    ...
    with exn ->
      let fallback =
        Document.make_with_profile ~profile:Parser.Background ~uri ~file
          ~text:""
      in
...
let quick_nav_metadata_of_skeleton_symbol
  ...
  in
  {
    ...
    external_kind;
    decl_role =
      Workspace_symbol_metadata.decl_role_of_external_kind external_kind;
...
let quick_nav_index_step ?(should_stop : unit -> bool = fun () -> false)
  ...
  if budget_ms <= 0 then ()
    ...
    let rec loop () =
      Thread.yield ();
      if should_stop () then Perf_stats.tick "quick_nav.interrupted"
      else if Perf_stats.now_ms () >= deadline then ()
...
let apply_parse_result_path (ws : t) ~(pr_kind : parse_job_kind)
  ...
  if pr_epoch <> ws.parse_epoch then
    ...
    | Some doc ->
        let doc =
          match pr_kind with
          | ParseJobHighLarge | ParseJobRootLarge -> doc
          | ParseJobNormalLarge ->
              if ws.startup_fully_nav_ready_ms <> None then
...
let finish_open_doc_now_if_needed (ws : t) ~(uri : T.DocumentUri.t) : bool =
  ...
  | Some doc -> (
      ...
      else if is_large_doc doc ws then (
        Perf_stats.tick
          (if is_huge_doc doc ws then "parse.open_doc_forced_skipped_huge"
           else "parse.open_doc_forced_skipped_large");
        false)
      else
        match doc.Document.file with
        | None -> false
          ...
          | _ -> (
              ...
              | Some text ->
                  let doc0 =
                    ...
                    with exn ->
                      let fallback =
                        Document.make_with_profile ~profile:Parser.Background
                          ~uri ~file:(Some path) ~text:""
                      in
                      with_internal_phase_diag fallback ~phase:"bg-open-doc"
                        ~exn
                  in
                  Some (background_doc_for_indexing ws doc0))
        in
        match doc_opt with
        | None -> ()
        | Some doc ->
...
let inline_symbol_hint_max_cached_bytes = 1_000_000
let startup_large_parse_idle_quiet_ms = 5_000
let startup_huge_open_parse_idle_quiet_ms = 30_000

let path_is_open_huge_doc (ws : t) ~(path : string) : bool =
  match find_open_doc_for_path ws ~path with
  | Some doc -> is_huge_doc doc ws
  | None -> false

let cached_symbol_hint_bytes (ws : t) : int =
  let seen = Hashtbl.create 512 in
  let add_doc key doc total =
    if key = "" || Hashtbl.mem seen key then total
    else (
      Hashtbl.replace seen key true;
      total + String.length doc.Document.text)
  in
  let total =
    Hashtbl.fold
      (fun uri doc total -> add_doc (Uri_path.docuri_to_string uri) doc total)
      ws.docs 0
  in
  Hashtbl.fold (fun key doc total -> add_doc key doc total) ws.files total

let maybe_build_symbol_hints_background (ws : t) : unit =
  if ws.symbol_hints <> None then ()
  else if cached_symbol_hint_bytes ws > inline_symbol_hint_max_cached_bytes then
    Perf_stats.tick "bg.hint_build_deferred_large_cache"
  else
    match ws.index with
    | None -> ()
    | Some idx ->
...
let background_tick ?(should_stop : unit -> bool = fun () -> false) (ws : t)
    ~(budget_ms : int) ~(mode : bg_tick_mode) ~(idle_quiet_ms : int)
    ~(last_message_ms : float) : unit =
  if budget_ms <= 0 then ()
  else
    ...
    let stop_requested () =
      ...
      if should_stop () then (
        if not !interrupted then (
          interrupted := true;
          Perf_stats.tick "bg.tick_interrupted");
        true)
      else false
    in
    ...
    ignore
      (enqueue_pending_open_doc_parses ws ~reason_group:"foreground_catchup");
    ...
    let parse_result_budget =
      ...
      | BgTickIdle -> 6
    in
    drain_parse_worker_results ws ~open_only:foreground_pending_before_results
      ~max_items:parse_result_budget;
    if stop_requested () then ()
    else
      let foreground_pending = has_pending_open_parse_work ws in
      if foreground_pending then
        Perf_stats.tick "sched.open_doc_foreground_gate"
      else (
        ...
        else if ws.bg_seed_needs_refresh then refresh_bg_seed_paths ws);
      ...
      if foreground_pending then Perf_stats.tick "quick_nav.deferred_open_doc"
        ...
        quick_nav_index_step ws ~should_stop:stop_requested
          ~budget_ms:quick_nav_budget;
      let budget =
        match workspace_pressure_mode ws with
      ...
      let large_parse_quiet_elapsed =
        strict_startup_drain
        || since_last_msg >= float_of_int effective_large_parse_quiet_ms
      in
      let huge_open_parse_quiet_elapsed =
        since_last_msg >= float_of_int startup_huge_open_parse_idle_quiet_ms
      in
      let startup_closed_large_parse_ready =
        (* For large workspaces, quick-nav readiness is the startup finish line.
      ...
      let allow_high_large =
        ...
        && (foreground_pending || startup_closed_large_parse_ready)
      in
      let allow_normal_large =
        match mode with
        | BgTickInteractive ->
            (not foreground_pending) && startup_closed_large_parse_ready
            && large_parse_quiet_elapsed
            && ws.startup_fully_nav_ready_ms = None
            && Queue.is_empty ws.bg_high_large_queue
      ...
      let allow_root_large =
        match mode with
        | BgTickInteractive ->
            startup_closed_large_parse_ready && large_parse_quiet_elapsed
            && (ws.startup_diag_hover_ready_ms = None
               || Hashtbl.length ws.open_parse_generation > 0)
        | BgTickIdle ->
            startup_closed_large_parse_ready && large_parse_quiet_elapsed
      ...
      while
        !keep && (not (stop_requested ())) && Perf_stats.now_ms () < deadline
      do
        ...
        match
          dequeue_bg_path ws ~mode ~allow_normal_large ~allow_root_large
            ~allow_high_large ~open_only:foreground_open_only ~prefer_open
        with
        | None -> keep := false
        | Some (path, prio) ->
            ...
            if
              ...
              && workspace_pressure_mode ws = PressureCritical
            then (
              enqueue_bg_path ws ~lane:LaneSweep
                ~reason_group:"pressure_backoff" ~high:false path;
              keep := false)
            else (
              (match prio with
              ...
              | BgQueueHighLarge | BgQueueRootLarge | BgQueueNormalLarge ->
                  if
                    path_is_open_huge_doc ws ~path
                    && not huge_open_parse_quiet_elapsed
                  then (
                    Perf_stats.tick "bg.parse.huge_open_deferred_busy";
                    enqueue_bg_path ws ~lane:LaneOpen
                      ~reason_group:"huge_open_idle" ~high:true path;
                    keep := false)
                  else if
                    prio = BgQueueNormalLarge
                    && ws.startup_fully_nav_ready_ms = None
                    && (not (has_open_doc_for_path_key ws ~path_key))
                    && not (startup_requires_background_drain ws)
                  then (
                    Perf_stats.tick "bg.parse.large_deferred_startup";
                    let lane =
                      match prio with
                      | BgQueueRootLarge -> LaneRoot
                      | BgQueueHighLarge -> LaneRoot
                      | BgQueueNormalLarge -> LaneSweep
                      | BgQueueHighSmall | BgQueueRootSmall | BgQueueNormalSmall
                        ->
                          LaneSweep
                    in
                    enqueue_bg_path ws ~lane ~high:(prio = BgQueueRootLarge)
                      path;
                    keep := false)
                  else if try_submit_large_parse_job ws ~path ~queue_kind:prio
                  then ()
                  else
                    let lane =
                      match prio with
      ...
      if not (stop_requested ()) then update_startup_ready_state ws
...
let drain_open_diag_revalidate_uris (ws : t) ~(max_items : int) :
  ...
  if max_items <= 0 then []
    ...
    let hint_ready_reason reason =
      match String.lowercase_ascii (String.trim reason) with
      | "hint" | "hint_ready" | "xmodule_ready" -> true
      | _ -> false
    in
    let current_open_doc uri =
      match Hashtbl.find_opt ws.docs uri with
    ...
    let rec loop n acc =
      if n <= 0 || Queue.is_empty ws.open_diag_revalidate_updates then
        ...
        | Some (uri, _reason) -> (
            ...
            if
              ws.startup_fully_nav_ready_ms <> None
              && hint_ready_reason _reason && current_open_doc uri
            then (
              Perf_stats.tick "diag.open.revalidate_skipped_ready_doc";
              loop n acc)
            else
              match
                refresh_open_doc_diags ~import_lookup_pump:false ws ~uri
