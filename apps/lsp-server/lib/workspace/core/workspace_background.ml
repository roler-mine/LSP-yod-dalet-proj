...
let parse_doc_worker ~(max_bytes : int) ~(uri : T.DocumentUri.t)
  ...
  if is_parse_guard_exceeded ~max_bytes ~text_len:(String.length text) then
    ...
    try Document.make ~uri ~file ~text
    with exn ->
    ...
    | Some job ->
        let result =
          ...
          | ParseJobOpen { path_key; uri; file; text; generation } ->
              let doc =
                parse_doc_worker ~max_bytes:ws.parse_file_max_bytes ~uri ~file
                  ~text
              in
              ParseResultOpen
                {
          ...
          | ParseJobPath { path; path_key } ->
              ...
              let doc_opt =
                ...
                | Some text ->
                    Some
                      (parse_doc_worker ~max_bytes:ws.parse_file_max_bytes ~uri
                         ~file:(Some path) ~text)
              in
              ParseResultPath
                {
            ...
            if open_doc.Document.parse_rev = open_doc.Document.rev then open_doc
              ...
              try
                Perf_stats.time "parse.open_doc_deferred" (fun () ->
                    parse_guarded_document_make ws ~uri:open_doc.Document.uri
                      ~file:open_doc.Document.file ~text:open_doc.Document.text)
              with exn ->
...
let background_tick (ws : t) ~(budget_ms : int) ~(mode : bg_tick_mode)
  ...
  if budget_ms <= 0 then ()
    ...
    let quick_nav_budget =
      match mode with
      | BgTickInteractive ->
          if diag_stage_pending then max 1 (budget_ms / 6)
