...
let strip_known_ext (s : string) : string =
  ...
  let rec go = function
    ...
    | ext :: tl ->
        ...
        if n >= m && String.sub lower (n - m) m = ext then String.sub s 0 (n - m)
        else go tl
  in
  go known_exts
  ...
  Lexer.with_session_state (fun () ->
      ...
      let rec gather acc =
        try
          ...
          match tok with Parser.EOF -> List.rev acc | _ -> gather acc
        with _ -> List.rev acc
      in
    ...
    let rec go j steps =
      if j >= len || steps > 12 then None
        ...
        | (Parser.LPAREN | Parser.RPAREN | Parser.COMMA), _, _ ->
            go (j + 1) (steps + 1)
        | Parser.ID raw, sp, ep | Parser.STRINGLIT raw, sp, ep ->
...
let find_function_macro_match ~(s : string) ~(pos : int)
    ~(function_macros : define_macro list) : (int * string) option =
...
let find_object_macro_match ~(s : string) ~(pos : int)
    ~(object_macros : define_macro list) : (int * string) option =
  let rec loop = function
    | [] -> None
    | m :: tl ->
        if starts_with_ci s ~pos m.name then Some (String.length m.name, m.body)
        else loop tl
  in
  loop object_macros

let expand_macros_once ~(s : string) ~(object_macros : define_macro list)
    ~(function_macros : define_macro list) : string =
  let n = String.length s in
  let buf = Buffer.create (max 16 n) in
  let rec loop i =
    if i >= n then ()
...
let expand_macros_recursive ~(s : string) ~(object_macros : define_macro list)
    ~(function_macros : define_macro list) : string =
...
let expand_text_with_macros ~(text : string) ~(macros : define_macro list)
    ~(define_spans : (int * int) list) : string =
  if macros = [] then text
  else
    let object_macros =
      macros
