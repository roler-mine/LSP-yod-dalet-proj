...
module Debug = struct
  let string_of_token (t : Parser.token) : string =
    ...
    | Parser.COMPOOL -> "COMPOOL"
    | Parser.ICOMPOOL -> "ICOMPOOL"
...
let parse_text ~(file : string option) ~(dump_ast : bool) ~(text : string) :
    output =
  Lexer.with_session_state (fun () ->
      (* Clear stale recovery/warn diags from previous runs *)
      Parse_diags.clear ();

      if String.trim text = "" then { ast = None; diags = []; ast_dump = None }
      else
        ...
        let pending_tok :
            (Parser.token * Lexing.position * Lexing.position * string) option
            ref =
          ref None
        in
        ...
        let remember_token (t : Parser.token) (sp : Lexing.position)
        ...
        let mk_error_diag () : T.Diagnostic.t =
          ...
          | Some Parser.EOF ->
              diag_error loc
                ("Parse error: unexpected end of file." ^ expected_hint)
          | Some t ->
        ...
        let is_sync_token = function
          | Parser.SEMI | Parser.COMMA | Parser.END | Parser.TERM | Parser.EOF
            ->
              true
          | _ -> false
        in
        ...
        try
          ...
          | Some ast ->
              let ast_dump =
                if dump_ast then Some (Ast.Debug.to_string ast) else None
              in
              { ast = Some ast; diags = errs @ extra; ast_dump }
          | None ->
              let errs =
                match errs with [] -> [ mk_error_diag () ] | xs -> xs
              in
        ...
        | Lexer.Lex_error (msg, sp, ep) ->
            ...
            {
              ast = None;
              diags = errs @ extra @ [ diag_error loc ("Lex error: " ^ msg) ];
              ast_dump = None;
            }
        | exn ->
            let extra = parse_diags_to_lsp ~file in
